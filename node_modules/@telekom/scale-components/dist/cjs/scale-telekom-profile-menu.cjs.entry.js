...
const TelekomProfileMenu = class {
  ...
  printSignInMenu() {
    return (index.h("div", { class: "profile-menu-login" }, index.h("strong", null, this.serviceName), index.h("p", null, this.serviceDescription), index.h("scale-button", { href: this.loginUrl || LOGIN_DEFAULT }, this.loginLabel), index.h("div", { class: "footer" }, index.h("p", null, index.h("scale-link", { "omit-underline": "true", href: this.loginHelpUrl || LOGIN_HELP_DEFAULT }, this.loginHelpLabel)), index.h("div", { id: "signUp" }, index.h("p", null, this.registerHeadline), index.h("p", null, index.h("scale-link", { "omit-underline": "true", href: this.registerUrl || REGISTER_DEFAULT }, this.registerLabel))))));
  }
  printProfileTrigger() {
    if (!this.loggedIn) {
      return (index.h("scale-icon-user-file-user", { selected: this.menuOpen }));
    }
    // logged in
    return (index.h("scale-badge", { "no-dot": "true" }, index.h("scale-icon-user-file-user", { selected: this.menuOpen }), index.h("div", { slot: "dot", class: "mydot" }, index.h("scale-icon-action-checkmark", null))));
  }
  buildLogoutButton() {
    return {
      type: 'button',
  ...
  printLabel() {
    if (!this.accessibilityLabel) {
      return index.h("span", { class: "flyout-label" }, this.label);
    }
    return (index.h("div", { class: "flyout-label" }, index.h("span", { "aria-hidden": "true" }, this.label), index.h("span", { class: "visually-hidden" }, this.accessibilityLabel)));
  }
  render() {
    return (index.h(index.Host, null, index.h("scale-telekom-nav-item", { class: "user-menu-desktop" }, index.h("scale-menu-flyout", { direction: "bottom-left", "onScale-open": (event) => this.openMenu(event), "onScale-close": (event) => this.closeMenu(event), triggerHasPopup: false }, index.h("a", { href: "javascript:void(0);", slot: "trigger", role: "button", "aria-controls": "user-menu-desktop" }, this.printProfileTrigger(), this.printLabel()), index.h("scale-menu-flyout-list", { id: "user-menu-desktop", preventFlipVertical: true, role: "none" }, this.loggedIn && [
      index.h("app-navigation-user-menu", { hide: () => {
          this.userMenuDesktopTrigger.click();
        }, navigation: this.buildUserNavigation(), styles: this.buildDesktopMenuStyles() }),
    ], !this.loggedIn && [
      index.h("app-navigation-user-menu", { navigation: [] }, this.printSignInMenu()),
    ]), index.h("div", { slot: "trigger", class: "user-menu-trigger", ref: (el) => (this.userMenuDesktopTrigger = el) }))), index.h("scale-telekom-nav-item", { class: "user-menu-mobile" }, index.h("button", null, this.printProfileTrigger(), this.printLabel()), index.h("scale-telekom-nav-flyout", { variant: "mobile" }, index.h("scale-telekom-mobile-flyout-canvas", { appName: this.appName, closeButtonLabel: this.closeMenuAccessibilityLabel }, this.loggedIn && [
      index.h("app-navigation-user-menu", { slot: "mobile-main-nav", navigation: this.buildUserNavigation(), styles: this.buildMobileMenuStyles() }),
    ], !this.loggedIn && [
      index.h("app-navigation-user-menu", { slot: "mobile-main-nav", navigation: [] }, this.printSignInMenu()),
    ])))));
  }
...
exports.scale_telekom_profile_menu = TelekomProfileMenu;
