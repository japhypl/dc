...
import { a as isClickOutside } from "./utils-c4af5b47-5j7RUTeV.js";
import { a as platform, i as offset, n as computePosition, o as shift, r as flip, t as arrow } from "./floating-ui.dom.esm-43ac86e1-BRqXGRTG.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-tooltip.entry.js
...
var Tooltip = class {
	constructor(hostRef) {
		...
		*/
		this.update = async () => {
			if (this.disabled || this.triggerEl == null) return;
		...
		this.handleBlur = () => {
			if (this.hasTrigger("focus")) this.hideTooltip();
		};
		this.handleClick = () => {
			if (this.hasTrigger("click")) this.opened && !this.hasTrigger("focus") ? this.hideTooltip() : this.showTooltip();
		};
		this.handleFocus = () => {
			if (this.hasTrigger("focus")) this.showTooltip();
		};
		...
		this.handleMouseOver = () => {
			if (this.hasTrigger("hover")) this.showTooltip();
		};
		this.handleMouseOut = () => {
			if (!this.mouseOverTooltip) {
				if (this.hasTrigger("hover")) this.hideTooltip();
			}
		};
	...
	connectedCallback() {
		...
		if (this.hostElement.hasAttribute("open")) statusNote({
			tag: "deprecated",
	...
	handleOutsideClick(event) {
		if (isClickOutside(event, this.hostElement)) this.hideTooltip();
	}
	componentDidUpdate() {
		this.update();
		if (this.opened) this.showTooltip();
	}
	componentDidRender() {
		this.update();
	}
	async showTooltip() {
		...
		if (this.tooltipBeforeShow.emit().defaultPrevented) {
			this.opened = false;
			return;
		}
		this.opened = true;
	...
	async hideTooltip() {
		if (!this.opened) return;
		if (this.tooltipBeforeHide.emit().defaultPrevented) {
			this.opened = true;
			return;
		}
		this.opened = false;
	...
	render() {
		return h(Host, { onKeyDown: this.handleKeyDown }, this.styles && h("style", null, this.styles), h("span", {
			part: "trigger",
			"aria-describedby": this.componentId
		}, h("slot", null)), !this.disabled && h("div", {
			part: "tooltip",
			role: "tooltip",
		...
		}, h("slot", { name: "content" }, this.content), h("div", {
			"aria-hidden": "true",
			part: "arrow",
...
//# sourceMappingURL=scale-tooltip.entry-Bu-i-tiH.js.map