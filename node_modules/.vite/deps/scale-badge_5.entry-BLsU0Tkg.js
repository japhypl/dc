...
var Badge = class {
	...
	getAriaLabel() {
		return this.ariaLabelTranslation.replace(/\$count/g, `${this.count}`).replace(/\$label/g, `${this.label}`);
	}
	render() {
		return h(Host, null, h("span", {
			part: "base",
			"aria-label": this.count ? this.getAriaLabel() : this.label
		}, h("slot", null), h("slot", { name: "dot" }), h("span", {
			"aria-hidden": "true",
			part: classnames(`circle`, this.count ? `has-count` : "no-count")
		}, !this.count ? "" : this.formatter.format(this.count)), h("span", {
			"aria-hidden": "true",
			id: "raw-count",
			part: "visually-hidden"
		}, this.count)), h("span", {
			id: "label",
			part: classnames("label", this.labelVisuallyHidden && "visually-hidden"),
			"aria-hidden": "true"
		}, this.label));
	}
	get hostElement() {
...
var UserFileUser = class {
	...
	render() {
		...
		return h(Host, null, h("svg", Object.assign({
			...
			d: "M8.9 16.5l3.1 3.1 3.1-3.1h2.25a4.73 4.73 0 014.656 3.73l.044.22.2 1.05H1.8l.15-1.05a4.73 4.73 0 014.474-3.945l.226-.005H8.9zM12 2c3.5 0 6 2.5 6 6 0 3.4-2.4 7-6 7s-6-3.6-6-7c0-3.5 2.5-6 6-6z",
			"fill-rule": "evenodd"
		})) : h("g", null, h("path", {
			d: "M8.9 16.5l3.1 3.1 3.1-3.1h2.25a4.73 4.73 0 014.656 3.73l.044.22.2 1.05H1.8l.15-1.05a4.73 4.73 0 014.474-3.945l.226-.005H8.9zM12 2c3.5 0 6 2.5 6 6 0 3.4-2.4 7-6 7s-6-3.6-6-7c0-3.5 2.5-6 6-6zm0 1.5C9.4 3.5 7.5 5.4 7.5 8c0 2.65 1.8 5.5 4.5 5.5s4.5-2.85 4.5-5.5c0-2.6-1.9-4.5-4.5-4.5z",
			"fill-rule": "evenodd"
		})))));
	}
	get hostElement() {
...
var TelekomMobileFlyoutCanvas = class {
	...
	render() {
		return h(Host, null, h("div", { part: "base" }, h("div", { part: "header" }, h("slot", { name: "heading" }, h("h2", { part: "heading" }, this.appName)), h("a", {
			...
			part: "close-button"
		}, h("slot", { name: "close-icon" }, h("scale-icon-action-close", {
			decorative: true,
			size: 20
		})))), h("div", { part: "body" }, h("slot", { name: "row" }, h("slot", { name: "mobile-before-main-nav" }), h("slot", { name: "mobile-main-nav" }), h("slot", { name: "mobile-after-main-nav" }), h("div", { part: "meta" }, h("div", null, h("slot", { name: "mobile-meta-nav-external" }), h("slot", { name: "mobile-meta-nav" })), h("div", null, h("slot", { name: "mobile-lang-switcher" }))), h("slot", { name: "mobile-bottom" })))));
	}
	get hostElement() {
		return getElement(this);
	}
...
var TelekomNavItem$1 = class {
	constructor(hostRef) {
		...
		this.handlePointerIn = () => {
			...
			if (this.hostElement.querySelector("scale-telekom-mega-menu") !== null) this.hostElement.querySelector("scale-telekom-mega-menu").addEventListener("mouseleave", this.handlePointerOut);
		};
	...
	handleDocumentClick(event) {
		...
		if (isNotTrigger() && isNotWithin()) this.expanded = false;
	}
	...
	connectedCallback() {
		...
		} else this.triggerElement.addEventListener("click", this.handleTriggerClick);
	}
	...
	*/
	get triggerElement() {
		if (this.triggerSelector) return this.hostElement.ownerDocument.querySelector(this.triggerSelector);
		return this.hostElement.previousElementSibling;
	}
	render() {
		return h(Host, null, h("div", { part: classnames("base", this.animationState, `variant-${this.variant}`, { expanded: this.isExpanded }) }, h("slot", null)), h("div", {
			part: classnames("backdrop", this.animationState, `variant-${this.variant}`, { expanded: this.isExpanded }),
...
var TelekomNavItem = class {
	...
	activeChanged(newValue) {
		if (this.linkElement == null) return;
		if (this.variant === "lang-switcher" || this.variant === "main-nav") toggleAriaCurrent(this.linkElement, newValue, this.active ? "true" : "false");
	}
	connectedCallback() {
	...
	componentDidLoad() {
		...
		if (child && parentRole === "menu") child.setAttribute("role", "menuitem");
	}
	get linkElement() {
...
//# sourceMappingURL=scale-badge_5.entry-BLsU0Tkg.js.map