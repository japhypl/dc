...
var Table = class {
	...
	componentWillLoad() {
		if (this.showSort) this.hostElement.querySelectorAll("th").forEach((th) => {
	...
	componentWillUpdate() {
		this.hostElement.querySelectorAll("th").forEach((th) => {
			if (th.children.length === 0) {
				...
				if (this.showSort) this.addSortIndicator(th);
			}
		});
	}
	componentDidLoad() {
		const progressbar = this.hostElement.querySelectorAll("scale-progress-bar");
		if (progressbar) progressbar.forEach((el) => {
			el.showStatus = false;
		});
...
//#endregion
export { Table as scale_table };

//# sourceMappingURL=scale-table.entry-BywMGzxF.js.map