import { a as createEvent$1, l as registerInstance, n as Host$1, o as getElement$1, r as NAMESPACE, s as h$1 } from "./index-6d95a4bc-DlO5twzZ.js";
import { t as statusNote } from "./status-note-0089e9c9-bOOzjQuc.js";
import { t as classnames } from "./index-713f92a5-R2Q0UnM3.js";
import { n as emitEvent$1, r as generateUniqueId } from "./utils-c4af5b47-5j7RUTeV.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-date-picker.entry.js
var scopeId;
var contentRef;
var hostTagName;
...
var addHostEventListeners = (elm, hostRef, listeners, attachParentListeners) => {
	if (listeners) listeners.map(([flags, name, method]) => {
...
var hostListenerProxy = (hostRef, methodName) => (ev) => {
	try {
		if (hostRef.$flags$ & 256) hostRef.$lazyInstance$[methodName](ev);
	...
	if (style) {
		if (typeof style === "string") {
			...
			if (!appliedStyles.has(scopeId)) {
				if (styleContainerNode.host && (styleElm = styleContainerNode.querySelector(`[${HYDRATED_STYLE_ID}="${scopeId}"]`))) styleElm.innerHTML = style;
				else {
		...
		} else if (!styleContainerNode.adoptedStyleSheets.includes(style)) styleContainerNode.adoptedStyleSheets = [...styleContainerNode.adoptedStyleSheets, style];
	}
	return scopeId;
};
...
var updateElement = (oldVnode, newVnode, isSvgMode, memberName) => {
	...
	for (memberName in oldVnodeAttrs) if (!(memberName in newVnodeAttrs)) setAccessor(elm, memberName, oldVnodeAttrs[memberName], void 0, isSvgMode, newVnode.$flags$);
...
var createElm = (oldParentVNode, newParentVNode, childIndex, parentElm) => {
	...
	if (!useNativeShadowDom) {
		...
		if (newVNode.$tag$ === "slot") {
			if (scopeId) parentElm.classList.add(scopeId + "-s");
			newVNode.$flags$ |= newVNode.$children$ ? 2 : 1;
		}
	}
	if (newVNode.$text$ !== null) elm = newVNode.$elm$ = doc.createTextNode(newVNode.$text$);
	else if (newVNode.$flags$ & 1) elm = newVNode.$elm$ = doc.createTextNode("");
	else {
		if (!isSvgMode) isSvgMode = newVNode.$tag$ === "svg";
		elm = newVNode.$elm$ = doc.createElementNS(isSvgMode ? SVG_NS : HTML_NS, newVNode.$flags$ & 2 ? "slot-fb" : newVNode.$tag$);
		if (isSvgMode && newVNode.$tag$ === "foreignObject") isSvgMode = false;
		updateElement(null, newVNode, isSvgMode);
		if (isDef(scopeId) && elm["s-si"] !== scopeId) elm.classList.add(elm["s-si"] = scopeId);
	...
	if (newVNode.$flags$ & 3) {
		...
		if (oldVNode && oldVNode.$tag$ === newVNode.$tag$ && oldParentVNode.$elm$) putBackInOriginalLocation(oldParentVNode.$elm$, false);
	}
	return elm;
};
...
var updateChildren = (parentElm, oldCh, newVNode, newCh) => {
	...
	while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) if (oldStartVnode == null) oldStartVnode = oldCh[++oldStartIdx];
		...
		if (oldStartVnode.$tag$ === "slot" || newEndVnode.$tag$ === "slot") putBackInOriginalLocation(oldStartVnode.$elm$.parentNode, false);
		patch(oldStartVnode, newEndVnode);
		parentElm.insertBefore(oldStartVnode.$elm$, oldEndVnode.$elm$.nextSibling);
		oldStartVnode = oldCh[++oldStartIdx];
		newEndVnode = newCh[--newEndIdx];
	} else if (isSameVnode(oldEndVnode, newStartVnode)) {
		if (oldStartVnode.$tag$ === "slot" || newEndVnode.$tag$ === "slot") putBackInOriginalLocation(oldEndVnode.$elm$.parentNode, false);
		...
		if (idxInOld >= 0) {
			...
			if (elmToMove.$tag$ !== newStartVnode.$tag$) node = createElm(oldCh && oldCh[newStartIdx], newVNode, idxInOld, parentElm);
			else {
		...
		if (node) parentReferenceNode(oldStartVnode.$elm$).insertBefore(node, referenceNode(oldStartVnode.$elm$));
	}
...
var isSameVnode = (vnode1, vnode2) => {
	if (vnode1.$tag$ === vnode2.$tag$) {
		if (vnode1.$tag$ === "slot") return vnode1.$name$ === vnode2.$name$;
...
var patch = (oldVNode, newVNode) => {
	...
	if (text === null) {
		...
		if (tag === "slot");
		else updateElement(oldVNode, newVNode, isSvgMode);
		if (oldChildren !== null && newChildren !== null) updateChildren(elm, oldChildren, newVNode, newChildren);
		else if (newChildren !== null) {
			if (oldVNode.$text$ !== null) elm.textContent = "";
			addVnodes(elm, null, newVNode, newChildren, 0, newChildren.length - 1);
		} else if (oldChildren !== null) removeVnodes(oldChildren, 0, oldChildren.length - 1);
		if (isSvgMode && tag === "svg") isSvgMode = false;
	} else if (defaultHolder = elm["s-cr"]) defaultHolder.parentNode.textContent = text;
		...
		if (childNode.nodeType === 1) {
			if (childNode["s-sr"]) {
				...
				for (j = 0; j < ilen; j++) {
					...
					} else if (nodeType === 1 || nodeType === 3 && childNodes[j].textContent.trim() !== "") {
						childNode.hidden = true;
						break;
					}
				}
...
var relocateSlotContent = (elm) => {
	...
	for (; i < ilen; i++) {
			...
			for (j = hostContentNodes.length - 1; j >= 0; j--) {
				...
				if (!node["s-cn"] && !node["s-nr"] && node["s-hn"] !== childNode["s-hn"]) {
					if (isNodeLocatedInSlot(node, slotNameAttr)) {
						...
						if (relocateNodeData) relocateNodeData.$slotRefNode$ = childNode;
						else relocateNodes.push({
		...
		if (childNode.nodeType === 1) relocateSlotContent(childNode);
	}
};
var isNodeLocatedInSlot = (nodeToRelocate, slotNameAttr) => {
	if (nodeToRelocate.nodeType === 1) {
		if (nodeToRelocate.getAttribute("slot") === null && slotNameAttr === "") return true;
		if (nodeToRelocate.getAttribute("slot") === slotNameAttr) return true;
		return false;
	}
	if (nodeToRelocate["s-sn"] === slotNameAttr) return true;
	return slotNameAttr === "";
};
			...
			if (relocateData.$slotRefNode$) {
				...
				if (!insertBeforeNode && parentNodeRef !== nodeToRelocate.parentNode || nodeToRelocate.nextSibling !== insertBeforeNode) {
					if (nodeToRelocate !== insertBeforeNode) {
						if (!nodeToRelocate["s-hn"] && nodeToRelocate["s-ol"]) nodeToRelocate["s-hn"] = nodeToRelocate["s-ol"].parentNode.nodeName;
						parentNodeRef.insertBefore(nodeToRelocate, insertBeforeNode);
					}
				}
			} else if (nodeToRelocate.nodeType === 1) nodeToRelocate.hidden = true;
		}
	}
...
var dispatchHooks = (hostRef, isInitialLoad) => {
	...
	} else promise = safeCall(instance, "componentWillUpdate");
...
var initializeClientHydrate = (hostElm, tagName, hostId, hostRef) => {
	...
	if (!plt.$orgLocNodes$) initializeDocumentHydrate(doc.body, plt.$orgLocNodes$ = /* @__PURE__ */ new Map());
	...
	childRenderNodes.map((c) => {
		...
		const node = c.$elm$;
		if (orgLocationNode && orgLocationNode["s-en"] === "") orgLocationNode.parentNode.insertBefore(node, orgLocationNode.nextSibling);
		if (!shadowRoot) {
			node["s-hn"] = tagName;
			if (orgLocationNode) {
	...
	if (shadowRoot) shadowRootNodes.map((shadowRootNode) => {
		if (shadowRootNode) shadowRoot.appendChild(shadowRootNode);
	});
	endHydrate();
};
var clientHydrate = (parentVNode, childRenderNodes, slotNodes, shadowRootNodes, hostElm, node, hostId) => {
	...
	if (node.nodeType === 1) {
		...
		for (i = node.childNodes.length - 1; i >= 0; i--) clientHydrate(parentVNode, childRenderNodes, slotNodes, shadowRootNodes, hostElm, node.childNodes[i], hostId);
		if (node.shadowRoot) for (i = node.shadowRoot.childNodes.length - 1; i >= 0; i--) clientHydrate(parentVNode, childRenderNodes, slotNodes, shadowRootNodes, hostElm, node.shadowRoot.childNodes[i], hostId);
		...
		if (childIdSplt[1] === hostId || childIdSplt[1] === "0") {
			...
			if (childNodeType === TEXT_NODE_ID) {
				...
				if (childNodeType === SLOT_NODE_ID) {
					childVNode.$tag$ = "slot";
					if (childIdSplt[5]) node["s-sn"] = childVNode.$name$ = childIdSplt[5];
					else node["s-sn"] = "";
					node["s-sr"] = true;
					if (shadowRootNodes) {
						childVNode.$elm$ = doc.createElement(childVNode.$tag$);
						if (childVNode.$name$) childVNode.$elm$.setAttribute("name", childVNode.$name$);
					...
					if (!parentVNode.$children$) parentVNode.$children$ = [];
					parentVNode.$children$[childVNode.$index$] = childVNode;
				} else if (childNodeType === CONTENT_REF_ID) if (shadowRootNodes) node.remove();
				else {
					hostElm["s-cr"] = node;
					node["s-cn"] = true;
				}
...
var initializeDocumentHydrate = (node, orgLocNodes) => {
	if (node.nodeType === 1) {
		let i = 0;
		for (; i < node.childNodes.length; i++) initializeDocumentHydrate(node.childNodes[i], orgLocNodes);
		if (node.shadowRoot) for (i = 0; i < node.shadowRoot.childNodes.length; i++) initializeDocumentHydrate(node.shadowRoot.childNodes[i], orgLocNodes);
	} else if (node.nodeType === 8) {
...
var parsePropertyValue = (propValue, propType) => {
	if (propValue != null && !isComplexType(propValue)) {
		...
		if (propType & 2) return parseFloat(propValue);
...
var setValue = (ref, propName, newVal, cmpMeta) => {
	...
	if ((!(flags & 8) || oldVal === void 0) && didValueChange) {
		...
		if (instance) {
			...
			if ((flags & 18) === 2) scheduleUpdate(hostRef, false);
		}
	}
};
var proxyComponent = (Cstr, cmpMeta, flags) => {
	if (cmpMeta.$members$) {
		if (Cstr.watchers) cmpMeta.$watchers$ = Cstr.watchers;
		const members = Object.entries(cmpMeta.$members$);
		const prototype = Cstr.prototype;
		members.map(([memberName, [memberFlags]]) => {
			if (memberFlags & 31 || flags & 2 && memberFlags & 32) Object.defineProperty(prototype, memberName, {
				get() {
			...
			else if (flags & 1 && memberFlags & 64) Object.defineProperty(prototype, memberName, { value(...args) {
		...
		if (flags & 1) {
			...
			Cstr.observedAttributes = members.filter(([_, m]) => m[0] & 15).map(([propName, m]) => {
				...
				if (m[0] & 512) cmpMeta.$attrsToReflect$.push([propName, attrName]);
				return attrName;
			});
		}
	}
...
var connectedCallback = (elm) => {
	if ((plt.$flags$ & 1) === 0) {
		...
		if (!(hostRef.$flags$ & 1)) {
			...
			if (!hostId) {
				if (cmpMeta.$flags$ & 12) setContentReference(elm);
			}
			{
				let ancestorComponent = elm;
				while (ancestorComponent = ancestorComponent.parentNode || ancestorComponent.host) if (ancestorComponent.nodeType === 1 && ancestorComponent.hasAttribute("s-id") && ancestorComponent["s-p"] || ancestorComponent["s-p"]) {
...
var proxyCustomElement = (Cstr, compactMeta) => {
	...
	return proxyComponent(Cstr, cmpMeta, 3);
};
...
function createDate(year, month, day) {
	...
	if (Number.isInteger(yearInt) && Number.isInteger(monthInt) && Number.isInteger(dayInt) && monthInt > 0 && monthInt <= 12 && dayInt > 0 && dayInt <= 31 && yearInt > 0) return new Date(yearInt, monthInt - 1, dayInt);
}
/**
...
function parseISODate(value) {
	...
	if (matches) return createDate(matches[1], matches[2], matches[3]);
}
/**
...
function getViewOfMonth(date, firstDayOfWeek = DaysOfWeek.Monday) {
	return getDaysInRange(startOfWeek(startOfMonth(date), firstDayOfWeek), endOfWeek(endOfMonth(date), firstDayOfWeek));
}
/**
* Form random hash
*/
...
var DatePickerInput = ({ onClick, dateFormatter, localization, name, formattedValue, valueAsDate, value, identifier, disabled, required, role, buttonRef, inputRef, onInput, onBlur, onFocus }) => {
	return h("div", { class: "duet-date__input-wrapper" }, h("input", {
		...
		ref: buttonRef,
		type: "button"
	}, h("span", { class: "duet-date__toggle-icon" }, h("svg", {
		"aria-hidden": "true",
		height: "24",
		viewBox: "0 0 21 21",
		width: "24",
		...
		"stroke-linecap": "round",
		"stroke-linejoin": "round"
	}), h("g", { fill: "currentColor" }, h("circle", {
		cx: "8.5",
		cy: "8.5",
		r: "1"
	}), h("circle", {
		...
		cy: "12.5",
		r: "1"
	}))))), h("span", { class: "duet-date__vhidden" }, localization.buttonLabel, valueAsDate && h("span", null, ", ", localization.selectedDateMessage, " ", dateFormatter.format(valueAsDate)))));
};
var DatePickerDay = ({ focusedDay, today, day, onDaySelect, onKeyboardNavigation, focusedDayRef, inRange, isSelected, dateFormatter }) => {
	...
	return h("button", {
		...
		ref: (el) => {
			if (isFocused && el && focusedDayRef) focusedDayRef(el);
		}
	}, h("span", { "aria-hidden": "true" }, day.getDate()), h("span", { class: "duet-date__vhidden" }, dateFormatter.format(day)));
};
...
function mapWithOffset(array, startingOffset, mapFn) {
	return array.map((_, i) => {
		return mapFn(array[(i + startingOffset) % array.length]);
	});
}
var DatePickerMonth = ({ selectedDate, focusedDate, labelledById, localization, firstDayOfWeek, min, max, dateFormatter, onDateSelect, onKeyboardNavigation, focusedDayRef }) => {
	const today = /* @__PURE__ */ new Date();
	const days = getViewOfMonth(focusedDate, firstDayOfWeek);
	return h("table", {
		class: "duet-date__table",
		"aria-labelledby": labelledById
	}, h("thead", null, h("tr", null, mapWithOffset(localization.dayNames, firstDayOfWeek, (dayName) => h("th", {
		class: "duet-date__table-header",
		scope: "col"
	}, h("span", { "aria-hidden": "true" }, dayName.substr(0, 2)), h("span", { class: "duet-date__vhidden" }, dayName))))), h("tbody", null, chunk(days, 7).map((week) => h("tr", { class: "duet-date__row" }, week.map((day) => h("td", { class: "duet-date__cell" }, h(DatePickerDay, {
		day,
		today,
...
var DuetDatePicker = class extends HTMLElement {
	constructor() {
		...
		*/
		...
		this.handleEscKey = (event) => {
			if (event.keyCode === keyCode.ESC) this.hide();
		};
		this.handleBlur = (event) => {
			event.stopPropagation();
			this.duetBlur.emit({ component: "duet-date-picker" });
		};
		this.handleFocus = (event) => {
			event.stopPropagation();
			this.duetFocus.emit({ component: "duet-date-picker" });
		};
		...
		this.handleTouchEnd = (event) => {
			...
			if (isHorizontalSwipe) this.addMonths(distX < 0 ? 1 : -1);
		...
		this.handleKeyboardNavigation = (event) => {
			...
			switch (event.keyCode) {
				...
				case keyCode.PAGE_UP:
					if (event.shiftKey) this.addYears(-1);
					else this.addMonths(-1);
					break;
		...
		this.handleDaySelect = (_event, day) => {
			if (!inRange(day, parseISODate(this.min), parseISODate(this.max))) return;
			if (day.getMonth() === this.focusedDay.getMonth()) {
				this.setValue(day);
				this.hide();
			} else this.setFocusedDay(day);
		};
		...
		this.handleInputChange = (e) => {
			...
			if (parsed || target.value === "") this.setValue(parsed);
		};
		this.processFocusedDayNode = (element) => {
			this.focusedDayNode = element;
			if (this.activeFocus && this.open) setTimeout(() => element.focus(), 0);
		};
	}
	connectedCallback() {
	...
	*/
	handleDocumentClick(e) {
		if (!this.open) return;
		const target = e.target;
		if (this.dialogWrapperNode.contains(target) || this.datePickerButton.contains(target)) return;
		this.hide(false);
	}
	/**
	* Public methods API
	...
	*/
	async hide(moveFocusToButton = true) {
		this.open = false;
		clearTimeout(this.focusTimeoutId);
		if (moveFocusToButton) setTimeout(() => this.datePickerButton.focus(), TRANSITION_MS + 200);
	}
	addDays(days) {
	...
	*/
	render() {
		...
		return h(Host, null, h("div", { class: "duet-date" }, h(DatePickerInput, {
		...
		}, h("label", { class: "duet-date__mobile-heading" }, this.localization.calendarHeading), h("button", {
			...
			class: "duet-date__vhidden"
		}, this.localization.monthSelectLabel), h("div", { class: "duet-date__select" }, h("select", {
			id: this.monthSelectId,
			class: "duet-date__select--month",
			ref: (element) => this.monthSelectNode = element,
			onChange: this.handleMonthSelect
		}, this.localization.monthNames.map((month, i) => h("option", {
			key: month,
			value: i,
			...
			height: "16",
			viewBox: "0 0 24 24"
		}, h("path", { d: "M8.12 9.29L12 13.17l3.88-3.88c.39-.39 1.02-.39 1.41 0 .39.39.39 1.02 0 1.41l-4.59 4.59c-.39.39-1.02.39-1.41 0L6.7 10.7c-.39-.39-.39-1.02 0-1.41.39-.38 1.03-.39 1.42 0z" })))), h("label", {
			htmlFor: this.yearSelectId,
			class: "duet-date__vhidden"
		}, this.localization.yearSelectLabel), h("div", { class: "duet-date__select" }, h("select", {
			...
			height: "16",
			viewBox: "0 0 24 24"
		}, h("path", { d: "M8.12 9.29L12 13.17l3.88-3.88c.39-.39 1.02-.39 1.41 0 .39.39.39 1.02 0 1.41l-4.59 4.59c-.39.39-1.02.39-1.41 0L6.7 10.7c-.39-.39-.39-1.02 0-1.41.39-.38 1.03-.39 1.42 0z" }))))), h("div", { class: "duet-date__nav" }, h("button", {
			...
			height: "21",
			viewBox: "0 0 24 24"
		}, h("path", { d: "M14.71 15.88L10.83 12l3.88-3.88c.39-.39.39-1.02 0-1.41-.39-.39-1.02-.39-1.41 0L8.71 11.3c-.39.39-.39 1.02 0 1.41l4.59 4.59c.39.39 1.02.39 1.41 0 .38-.39.39-1.03 0-1.42z" })), h("span", { class: "duet-date__vhidden" }, this.localization.prevMonthLabel)), h("button", {
			...
			height: "21",
			viewBox: "0 0 24 24"
		}, h("path", { d: "M9.29 15.88L13.17 12 9.29 8.12c-.39-.39-.39-1.02 0-1.41.39-.39 1.02-.39 1.41 0l4.59 4.59c.39.39.39 1.02 0 1.41L10.7 17.3c-.39.39-1.02.39-1.41 0-.38-.39-.39-1.03 0-1.42z" })), h("span", { class: "duet-date__vhidden" }, this.localization.nextMonthLabel)))), h(DatePickerMonth, {
...
var DuetDatePicker$1 = /* @__PURE__ */ proxyCustomElement(DuetDatePicker, [
	0,
	"duet-date-picker",
	{
		"name": [1],
...
if (typeof window !== "undefined" && typeof window.Audio !== "undefined" && !customElements.get("duet-date-picker")) customElements.define("duet-date-picker", DuetDatePicker$1);
...
var DatePicker = class {
	constructor(hostRef) {
		...
		*/
		this.adjustButtonsLabelsForA11y = () => {
			...
			const callback = () => {
				this.mo.disconnect();
				Array.from(this.hostElement.querySelectorAll(".duet-date__day")).forEach((button) => {
	...
	*/
	onPlaceholderChange(newValue) {
		const input = this.duetInput.querySelector(".duet-date__input");
		if (input && this.placeholder) input.setAttribute("placeholder", newValue);
	}
	componentWillLoad() {
		if (this.popupTitle !== "Pick a date") statusNote({
			tag: "deprecated",
		...
		if (this.identifier == null) this.identifier = "scale-date-picker-" + this.internalId;
	}
	componentDidLoad() {
		var _a, _b;
		...
		if (selectIcon) Array.from(selectIcon).forEach((icon) => icon.replaceWith(document.createElement("scale-icon-navigation-collapse-down")));
		const input = this.duetInput.querySelector(".duet-date__input");
		if (input) input.addEventListener("keyup", this.handleKeyPress);
		if (input && (this.helperText || this.ariaDetailsId)) {
			const describedBy = this.helperText ? `helper-message-${this.internalId}` : this.ariaDetailsId;
			input.setAttribute("aria-describedby", describedBy);
		}
		if (input && this.ariaDetailsId) input.setAttribute("aria-details", this.ariaDetailsId);
		if (input && this.placeholder) input.setAttribute("placeholder", this.placeholder);
		if (input && (this.status === "error" || this.invalid)) input.setAttribute("aria-invalid", "true");
		...
		if (dialog) {
			...
			if (duetHeadingId) {
				...
				if (duetHeading) duetHeading.parentElement.removeChild(duetHeading);
			}
		}
		...
		if (tableHeadings) Array.from(tableHeadings).forEach((item) => item.innerHTML = item.innerHTML[0]);
		const today = this.hostElement.querySelector(".duet-date__day.is-today span.duet-date__vhidden");
		if (today) today.innerHTML = `${today.innerHTML}, ${((_b = this.localization) === null || _b === void 0 ? void 0 : _b.today) || "today"}`;
		this.adjustButtonsLabelsForA11y();
	}
	componentDidRender() {
		if (this.status !== "") statusNote({
			tag: "deprecated",
			message: "Property \"status\" is deprecated. Please use the \"invalid\" property!",
			type: "warn",
			source: this.hostElement
		});
		if (this.size) statusNote({
			tag: "deprecated",
	...
	disconnectedCallback() {
		if (this.duetInput) {
			const input = this.duetInput.querySelector(".duet-date__input");
			if (input) input.removeEventListener("keyup", this.handleKeyPress);
		}
	...
	render() {
		return h$1(Host$1, null, this.styles && h$1("style", null, this.styles), h$1("div", { class: classnames("scale-date-picker", this.status && `scale-date-picker--status-${this.status}`, this.invalid && `scale-date-picker--status-error`, this.hasFocus && "scale-date-picker--focus", this.disabled && "scale-date-picker--disabled", this.hasValue && "animated", this.helperText && "has-helper-text") }, h$1("label", {
		...
		}), this.helperText && h$1("scale-helper-text", {
...
//#endregion
export { DatePicker as scale_date_picker };

//# sourceMappingURL=scale-date-picker.entry-D1IwYTKP.js.map