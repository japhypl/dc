...
//#region node_modules/@telekom/scale-components/dist/esm/scale-toggle-button.entry.js
...
var ToggleButton = class {
	constructor(hostRef) {
		...
		this.handleIconShape = () => {
			if (this.hasScaleIcon) Array.from(this.hostElement.children).forEach((node) => {
				if (node.nodeName.substr(0, 10) === "SCALE-ICON") if (this.selected) node.setAttribute("selected", "true");
	...
	componentDidRender() {
		this.handleIconSize();
		if (this.hostElement.hasAttribute("aria-label")) statusNote({
			tag: "deprecated",
	...
	componentWillLoad() {
		if (this.toggleButtonId == null) this.toggleButtonId = "toggle-button-" + i++;
	}
	...
	handleIconSize() {
		Array.from(this.hostElement.children).forEach((child) => {
			if (child.tagName.substr(0, 10) === "SCALE-ICON") child.setAttribute("size", iconSizes[this.size]);
		});
	}
	/**
	...
	*/
	setIconPositionProp() {
		const nodes = Array.from(this.hostElement.childNodes).filter((node) => {
			if (node.nodeName.substr(0, 10) === "SCALE-ICON") this.hasScaleIcon = true;
			return !(node.nodeType === 3 && node.nodeValue.trim() === "");
		});
		if (!this.iconOnly && nodes && nodes.length && nodes[nodes.length - 1] && nodes[nodes.length - 1].nodeName.substr(0, 10) === "SCALE-ICON") this.iconPosition = "after";
	}
	render() {
...
//# sourceMappingURL=scale-toggle-button.entry-D1xkIPRA.js.map