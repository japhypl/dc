...
import { s as isScaleIcon } from "./utils-c4af5b47-5j7RUTeV.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-tab-header.entry.js
...
var TabHeader = class {
	constructor(hostRef) {
		...
		this.scaleDisabled = createEvent(this, "scale-disabled", 7);
		this.generatedId = i++;
	...
	selectedChanged(newValue) {
		if (!this.hostElement.isConnected || this.disabled) return;
		if (newValue) this.scaleSelect.emit();
		if (newValue && this.tabsHaveFocus()) this.hostElement.focus();
	...
	*/
	setChildrenIconSize() {
		Array.from(this.hostElement.childNodes).filter(isScaleIcon).forEach((icon) => {
			if (icon.size !== PER_SPEC_ICON_SIZE) icon.size = PER_SPEC_ICON_SIZE;
		});
	}
	render() {
		return h(Host, {
...
//# sourceMappingURL=scale-tab-header.entry-BBm8Jx8h.js.map