...
import { t as animationsFinished } from "./utils-c4af5b47-5j7RUTeV.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-notification.entry.js
...
var Notification = class {
	constructor(hostRef) {
		...
		this.open = () => {
			...
			requestAnimationFrame(async () => {
				...
				this.scaleOpen.emit();
				if (this.delay !== void 0) setTimeout(this.timeout, this.delay);
			});
		};
		this.close = () => {
			const event = this.scaleBeforeClose.emit({ trigger: this.lastCloseEventTrigger });
			this.lastCloseEventTrigger = null;
	...
	connectedCallback() {
		if (this.hostElement.hasAttribute("opened")) {
			if (this.innerAriaLive === "polite" || this.innerRole === "status") this.innerRole = "status";
			this.isOpen = true;
		}
		if (this.delay !== void 0) setTimeout(this.timeout, this.delay);
	...
	render() {
		...
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", {
			part: classnames("base", this.animationState, `type-${this.type}`, `variant-${this.variant}`, this.isOpen && "open"),
			role: this.innerRole
		}, h("div", {
			part: "icon",
			"aria-hidden": "true"
		}, h("slot", { name: "icon" }, h(IconTag, {
			size: ICON_SIZE,
			selected: this.type === "toast"
		}))), h("div", { part: "body" }, h("div", {
			part: "heading",
			role: "heading",
			"aria-level": this.headingLevel,
			"aria-label": `${this.ariaHeading} ${this.heading}`
		}, this.heading ? h("span", null, this.heading) : null, h("slot", { name: "heading" })), this.hasTextSlot && h("div", { part: "text" }, h("slot", { name: "text" }))), this.dismissible && h("scale-button", {
			part: "close-button",
			variant: "ghost",
			onClick: () => {
		...
		}, h("slot", { name: "close-icon" }, h("scale-icon-action-circle-close", {
...
//# sourceMappingURL=scale-notification.entry-CRHhtWWm.js.map