...
import { n as findSelected, t as findRootNode } from "./menu-utils-ee192675-DzOday92.js";
import { t as classnames } from "./index-713f92a5-R2Q0UnM3.js";
import { t as renderIcon } from "./render-icon-05777d0c-C85XWt45.js";
//#region node_modules/@telekom/scale-components/dist/esm/app-mega-menu_7.entry.js
...
var MegaMenu = class {
	...
	componentWillRender() {
		if (this.isActive !== void 0) statusNote({
			tag: "deprecated",
	...
	render() {
		return h("div", { class: "mega-menu" }, h("div", { class: "mega-menu__wrapper" }, this.hasCustomBody ? h("slot", { name: "custom-body" }) : h("ul", { class: "mega-menu__container" }, this.navigation.map((child) => h("li", { class: "mega-menu__row" }, h("div", { class: "mega-menu__row-title" }, child.name), h("ul", null, child.children && child.children.length > 0 && child.children.map((menuItem) => h("li", null, h("a", {
			class: `mega-menu__row-item ${this.activeRouteId === menuItem.id ? "active" : ""}`,
			"aria-current": this.activeRouteId === menuItem.id ? "true" : "false",
			href: menuItem.href || "javascript:void(0);",
			tabIndex: this.active || this.isActive ? 0 : -1,
			onClick: (event) => {
				this.hide();
				if (typeof menuItem.onClick === "function") menuItem.onClick(event);
			},
			onKeyDown: (event) => {
				if (["Escape", "Esc"].includes(event.key)) this.hide();
			}
...
var MainNavigationMobile = class {
	...
	handleSelect(event, item) {
		...
		if (!hasChildren(selected)) this.closeMenuHandler();
	}
	childMenuPage() {
		...
		if (!section) return h("div", null);
		const isActive = (child) => selected && child.id === selected.id || parent && parent.id === child.id;
		return h("div", { class: "main-navigation-mobile__child-menu" }, h("a", {
			...
			onKeyDown: (event) => {
				if (["Enter", " "].includes(event.key)) {
					event.preventDefault();
					this.handlePrevSelected(event, section);
					if (!this.selected) this.mainNavigationWrapper.querySelector("a").focus();
				}
				if (["Escape", "Esc"].includes(event.key)) this.hide();
			}
		}, h("div", { class: "main-navigation-mobile__child-menu-current-item" }, h("div", { class: "main-navigation-mobile__child-menu-current-wrapper" }, h("scale-icon-navigation-left", null), h("div", null, section.name)))), h("ul", {
			class: "main-navigation-mobile__child-menu-items",
			ref: (el) => {
				this.childrenWrapper = el;
			}
		}, section.children.map((child) => h("li", { class: "main-navigation-mobile__child-menu-item" }, h("a", {
			"aria-current": isActive(child) ? "true" : "false",
			"aria-haspopup": hasChildren(child) ? "true" : "false",
			class: `main-navigation-mobile__child-menu-item-link ${isActive(child) ? "selected" : ""}`,
			...
			onKeyDown: (event) => {
				if (["Enter", " "].includes(event.key)) {
					this.handleSelect(event, child);
					setTimeout(() => {
						if (this.childrenWrapper.querySelector("a")) this.childrenWrapper.querySelector("a").focus();
					});
				}
				if (["Escape", "Esc"].includes(event.key)) this.hide();
			}
	...
	render() {
		...
		return h("div", { class: "main-navigation-mobile" }, this.childMenuPage(), h("ul", {
	...
	static get watchers() {
		return { "activeRouteId": ["handleActiveRoute"] };
	}
};
...
var NavigationSectorMobile = class {
	constructor(hostRef) {
		registerInstance(this, hostRef);
		this.selected = this.navigation ? this.navigation.find(({ id }) => id === this.activeSectorId) || this.navigation[0] : {};
	}
	...
	render() {
		return h("ul", { class: "sector-navigation-mobile" }, (this.navigation || []).map((item) => h("li", { class: "sector-navigation-mobile__item" }, h("a", {
			class: `sector-navigation-mobile__item-link${this.selected.id === item.id ? " sector-navigation-mobile__item-link--selected" : ""}`,
			href: item.href || "javascript:void(0);",
		...
		}, item.name, this.selected.id === item.id && h("span", { class: "sr-only" }, "active")))));
	}
	static get watchers() {
		return { "activeSectorId": ["handleActiveSegment"] };
	}
};
...
var NavIcon = class {
	...
	componentWillRender() {
		...
		if (this.isMobileMenuOpen !== false) statusNote({
			tag: "deprecated",
	...
	render() {
		return h("li", { class: this.getCssClassMap() }, h("a", {
			class: "meta-navigation__item-link",
			ref: this.refMobileMenuToggle || this.refMobileUserMenuToggle || this.refUserMenuToggle,
			href: this.href,
			role: this.href === "javascript:void(0);" ? "button" : null,
			onClick: this.clickLink,
			onKeyDown: (event) => {
				if (!this.refMobileMenuToggle) return;
				if ([
					"Enter",
					" ",
					"Escape",
		...
		}, this.badge || this.badgeLabel && this.badge || this.badgeLabel ? h("scale-notification-badge", {
...
var NavMain = class {
	...
	componentWillRender() {
		if (this.isMegaMenuVisible !== false) statusNote({
			tag: "deprecated",
	...
	render() {
		return h(Host, null, h("li", { class: this.getCssClassMap() }, h("a", {
			...
			onClick: this.clickLink,
			id: this.innerId
		}, h("span", { class: "main-navigation__item-link-text" }, this.name), (this.active || this.isActive) && h("span", { class: "sr-only" }, "active")), h("slot", null)));
	}
	getCssClassMap() {
...
var NavSegment = class {
	...
	render() {
		return h("li", { class: this.getCssClassMap() }, h("a", {
			class: classnames("segment-navigation__item-link", (this.active || this.isActive) && "active"),
			href: this.href,
			onFocus: () => {
				window.scrollTo({ top: 0 });
			},
			"aria-current": this.active || this.isActive ? "true" : "false"
		}, h("slot", null), (this.active || this.isActive) && h("span", { class: "sr-only" }, "active")));
	}
	getCssClassMap() {
...
var NotificationBadge = class {
		...
		if (this.label) {
			if (!isNaN(this.label)) {
				...
				if (labelNumber.length > this.maxCharacters) {
					...
					if (tier > 0) labelNumber = (Number(this.label) / Math.pow(10, tier * 3)).toFixed(1).replace(".0", "") + SI_SYMBOL[tier];
				}
				return labelNumber;
			}
			return this.label;
	...
	render() {
		return h(Host, null, this.type !== "nav-icon" ? h("div", {
...
//#endregion
export { MegaMenu as app_mega_menu, MainNavigationMobile as app_navigation_main_mobile, NavigationSectorMobile as app_navigation_sector_mobile, NavIcon as scale_nav_icon, NavMain as scale_nav_main, NavSegment as scale_nav_segment, NotificationBadge as scale_notification_badge };

//# sourceMappingURL=app-mega-menu_7.entry-CKeK3G67.js.map