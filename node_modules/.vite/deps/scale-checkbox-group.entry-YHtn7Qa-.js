...
//#region node_modules/@telekom/scale-components/dist/esm/scale-checkbox-group.entry.js
...
var CheckboxGroup = class {
	...
	handleCheckboxChange(ev) {
		...
		if (tagName.toLowerCase() === "scale-checkbox") if (el !== this.groupNode) this.updateParentCheckboxState();
		else {
	...
	componentDidLoad() {
		...
		mo.observe(fieldset, { childList: true });
		this.observer = mo;
	}
	...
	updateChildrenCheckboxStates(checked) {
		this.getChildNodes().filter((node) => !node.disabled).forEach((node) => {
			if (checked !== void 0) {
	...
	render() {
		return h(Host, { class: "checkbox-group" }, h("scale-checkbox", {
			ref: (el) => this.groupNode = el,
			name: this.name,
			label: this.label,
			ariaLabelCheckbox: `${this.ariaLabelCheckboxGroup || this.label} - ${this.actionText}`,
		...
		}), h("fieldset", { part: "fieldset" }, h("legend", null, this.ariaLabelCheckboxGroup || this.label), h("slot", null)));
	}
	get host() {
...
//# sourceMappingURL=scale-checkbox-group.entry-YHtn7Qa-.js.map