...
//#region node_modules/@telekom/scale-components/dist/esm/scale-toggle-group.entry.js
...
var ToggleGroup = class {
	...
	scaleClickHandler(ev) {
		let tempState;
		if (this.singleSelect) if (!ev.detail.selected) tempState = this.status.map((obj) => ev.detail.id === obj.id ? ev.detail : Object.assign({}, obj));
		else tempState = this.status.map((obj) => ev.detail.id === obj.id ? ev.detail : Object.assign(Object.assign({}, obj), { selected: false }));
		else tempState = this.status.map((obj) => ev.detail.id === obj.id ? ev.detail : Object.assign({}, obj));
	...
	*/
	...
	getAriaLabelTranslation() {
		return this.ariaLabelTranslation.replace(/\$slottedButtons/g, `${this.slottedButtons}`);
	}
	componentDidRender() {
		if (this.fullWidth) this.setButtonWidth();
		this.setChildrenCorners();
		if (this.colorScheme !== "color") statusNote({
			tag: "deprecated",
	...
	setNewState(tempState) {
		Array.from(this.hostElement.querySelectorAll("scale-toggle-button")).forEach((button, i) => {
	...
	setChildrenCorners() {
		...
		if (children.length === 1) children[0].setAttribute("radius", "all");
		else for (let i = 0; i < children.length; i++) {
			if (i === 0) children[i].setAttribute("radius", "left");
			if (i > 0 && i < children.length - 1) children[i].setAttribute("radius", "neither");
			if (i === children.length - 1) children[i].setAttribute("radius", "right");
		}
	}
	render() {
...
//# sourceMappingURL=scale-toggle-group.entry-D8u94V2Y.js.map