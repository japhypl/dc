...
//#region node_modules/@telekom/scale-components/dist/esm/scale-sidebar-nav.entry.js
...
var SidebarNav = class {
	...
	disconnectedCallback() {
		if (this.mq != null) this.mq.removeListener(this.handleMediaQueryChange);
	}
	componentDidRender() {
		if (this.el.hasAttribute("aria-label")) statusNote({
			tag: "deprecated",
	...
	*/
		...
		function setNestingLevel(el, level = 1) {
			Array.from(el.children).forEach((child) => {
				if (child.tagName.toUpperCase() === "SCALE-SIDEBAR-NAV-COLLAPSIBLE") {
					...
					if (!child.hasAttribute("nesting-level")) child.setAttribute("nesting-level", String(level));
					if (level === 2 && !child.hasAttribute("condensed")) child.setAttribute("condensed", "true");
				}
				if (child.tagName.toUpperCase() === "SCALE-SIDEBAR-NAV-ITEM") {
					if (!child.hasAttribute("nesting-level")) child.setAttribute("nesting-level", String(level));
					if (level === 3 && !child.hasAttribute("condensed")) child.setAttribute("condensed", "true");
				}
			});
		}
	...
	render() {
		...
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", {
			...
			size: 20
		})), h("nav", Object.assign({ part: "nav" }, label, hidden), h("ul", {
			part: "list",
...
//#endregion
export { SidebarNav as scale_sidebar_nav };

//# sourceMappingURL=scale-sidebar-nav.entry-D3AzUYm5.js.map