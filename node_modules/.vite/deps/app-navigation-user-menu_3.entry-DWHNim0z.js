...
import { t as renderIcon } from "./render-icon-05777d0c-C85XWt45.js";
import { a as isClickOutside, n as emitEvent } from "./utils-c4af5b47-5j7RUTeV.js";
//#region node_modules/@telekom/scale-components/dist/esm/app-navigation-user-menu_3.entry.js
...
var AppNavigationUserMenu = class {
	...
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", { class: "app-navigation-user-menu" }, h("slot", null), this.navigation.map((item) => {
			if (item.type === "divider") return h("hr", {
				class: "app-navigation-user-menu__divider",
				part: "rule-horizontal"
			});
			if (item.type === "userInfo") return h("div", {
				class: "app-navigation-user-menu__user-info",
				part: "userInfo"
			}, h("div", { class: "app-navigation-user-menu__user-info--name scl-font-variant-heading-4" }, item.name), h("div", { class: "app-navigation-user-menu__user-info--email" }, item.email));
			if (item.type === "item") return h("a", {
				...
				attributes: { class: `app-navigation-user-menu__item--icon-prefix` }
			}) : null, item.name, item.icon && item.iconPosition === "suffix" ? renderIcon({
				tag: `scale-icon-${item.icon}`,
				attributes: { class: `app-navigation-user-menu__item--icon-suffix` }
			}) : null);
			if (item.type === "button") return h("scale-button", {
...
var MENU_SELECTOR = ".scale-menu-flyout-list";
var isButtonOrLink = (el) => {
	if (el.tagName.toUpperCase() === "BUTTON" || el.tagName.toUpperCase() === "A" || el.getAttribute("role") === "button") return el;
};
var MenuFlyout = class {
	constructor(hostRef) {
		...
		this.toggle = () => {
			...
			if (this.direction != null) list.direction = this.direction;
	...
	handleScaleSelect({ detail }) {
		if (detail.closeOnSelect === false) return;
		if (this.closeOnSelect) window.requestAnimationFrame(() => {
			this.closeAll();
		});
	}
	handleScaleClose({ detail }) {
		const parent = detail.list != null ? detail.list.parentNode.closest(MENU_SELECTOR) : null;
		if (parent) window.requestAnimationFrame(() => {
			parent.active = true;
			parent.setFocus();
		});
	}
	...
	handleOutsideClick(event) {
		if (isClickOutside(event, this.hostElement)) this.closeAll();
	}
	handleKeydown(event) {
		if ("Tab" === event.key && !this.hostElement.querySelector("app-navigation-user-menu")) {
			if (this.trigger.tagName === "SCALE-TELEKOM-NAV-ITEM") this.trigger.firstElementChild.focus();
			this.closeAll();
			return;
		}
	}
	componentDidLoad() {
		...
		if (triggerSlot && tagName === "SCALE-BUTTON") this.trigger = triggerSlot.shadowRoot.querySelector("button");
		else if (triggerSlot && tagName === "SCALE-NAV-ICON") this.trigger = triggerSlot.querySelector("a");
		else this.trigger = triggerSlot;
	...
	setTriggerAttributes() {
		Array.from(this.hostElement.querySelectorAll("[role=\"menuitem\"]")).filter((el) => el.querySelector("[slot=\"sublist\"]") != null).concat([isButtonOrLink(this.trigger)]).filter((x) => x != null).forEach((el) => {
			if (this.triggerHasPopup) el.setAttribute("aria-haspopup", "true");
	...
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", {
			part: "trigger",
			onClick: this.toggle
		}, h("slot", { name: "trigger" })), h("slot", null));
	}
	get hostElement() {
		return getElement(this);
	}
...
var MenuFlyoutList = class {
	...
	componentDidLoad() {
		if (!this.hostElement.hasAttribute("role")) this.hostElement.setAttribute("role", "menu");
	}
	componentDidRender() {
	...
	async close(silent = false) {
		if (this.active && silent !== true) emitEvent(this, "scaleClose", { list: this.hostElement });
		this.opened = false;
	}
	async setFocus() {
		if (this.focusedItemIndex != null) this.focusItem();
		else this.setInitialItemsFocus();
	}
	handleResize() {
		this.close();
	}
	handleKeydown(event) {
		...
		if (!this.hostElement.querySelector("app-navigation-user-menu")) event.preventDefault();
		...
		if (" " === event.key || "Enter" === event.key || "ArrowRight" === event.key) {
			...
			if (item != null) item.triggerEvent(event, this.closeOnSelect);
		}
	}
	/**
	...
	*/
	handleScaleOpen({ detail }) {
		if (detail.list !== this.hostElement) this.active = false;
	}
	openedChanged() {
		if (!this.opened) {
			this.active = false;
			...
			if (this.trigger().tagName === "SCALE-TELEKOM-NAV-ITEM") this.trigger().style.color = "var(--telekom-color-text-and-icon-standard)";
		}
		if (this.opened) {
			this.active = true;
			this.setFocus();
	...
	setInitialItemsFocus() {
		...
		if (this.items.length > 0) this.shiftItemsFocus();
	}
	...
	updateTriggerAttributes() {
		...
		if (trigger && trigger.getAttribute("aria-haspopup") === "true" || trigger.classList.contains("scale-menu-trigger")) trigger.setAttribute("aria-expanded", String(this.opened));
	}
	setWindowSize() {
	...
	setPosition() {
		...
		} else this.hostElement.style.top = `${top}px`;
	}
	setSize() {
		const { width, height } = this.triggerRect;
		this.hostElement.style.height = `${height}px`;
		this.hostElement.style.width = `${width}px`;
		if (this.brandHeaderDropdown) this.base.style.width = `240px`;
	}
	checkPlacement() {
		...
		if (rect.left < PAD) {
			isOutOfBounds = true;
			if (this.direction.includes("left")) this.flipHorizontal = true;
		}
		if (rect.right > this.windowWidth - PAD) {
			isOutOfBounds = true;
			if (this.direction.includes("right")) this.flipHorizontal = true;
		}
		if (rect.top < PAD) {
			isOutOfBounds = true;
			if (this.direction.includes("top") && !this.preventFlipVertical) this.flipVertical = true;
		}
		if (rect.bottom > this.windowHeight - PAD) {
			isOutOfBounds = true;
			if (this.direction.includes("bottom") && !this.preventFlipVertical) this.flipVertical = true;
		}
		if (isOutOfBounds) this.furtherAdjustPlacement();
	}
	furtherAdjustPlacement() {
		...
		else if (rect.right > this.windowWidth - PAD) left = this.windowWidth - PAD - rect.right;
		if (rect.top < PAD) top = PAD - rect.top;
		else if (rect.bottom > this.windowHeight - PAD) top = this.windowHeight - PAD - rect.bottom;
	...
	*/
	...
	updateScrollIndicators() {
		...
		if (diff) {
			if (this.list.scrollTop > 0) this.canScrollUp = true;
			if (this.list.scrollTop < diff) this.canScrollDown = true;
		}
		this.forceRender++;
	}
	/**
	...
	*/
	stopWheelPropagation(event) {
		...
		if (!this.canScrollDown && !this.canScrollUp) event.preventDefault();
		if (event.deltaY > 0 && !this.canScrollDown) event.preventDefault();
		if (event.deltaY < 0 && !this.canScrollUp) event.preventDefault();
	}
	getListItems() {
	...
	render() {
		return h(Host, { class: "scale-menu-flyout-list" }, this.styles && h("style", null, this.styles), h("div", {
...
//#endregion
export { AppNavigationUserMenu as app_navigation_user_menu, MenuFlyout as scale_menu_flyout, MenuFlyoutList as scale_menu_flyout_list };

//# sourceMappingURL=app-navigation-user-menu_3.entry-DWHNim0z.js.map