...
var Carousel = class {
	constructor(hostRef) {
		...
		this.handleSlideChange = (direction) => {
			...
			if (direction === "prev") val === 0 ? this.value = -100 * (this.slidesArray.length - 1) : this.value = val + 100;
			if (direction === "next") val === -100 * (this.slidesArray.length - 1) ? this.value = 0 : this.value = val - 100;
		};
		this.setActiveSlide = (index) => {
			this.value = -100 * index;
		};
		this.setTransformValue = () => {
			if (!!this.vertical) return `translateY(${this.value}%)`;
			return `translateX(${this.value}%)`;
		};
		this.setActiveCssClass = (index) => {
			if (Math.abs(this.value) / 100 === index) return "carousel__indicator--active";
			return "";
		};
	}
	connectedCallback() {
	...
	componentWillLoad() {
		if (this.slidesArray.length === 0) {
			...
			for (let childIndex = 0; childIndex < children.length; childIndex++) if (children[childIndex].slot === "") for (let slideIndex = 0; slideIndex < children[childIndex].children.length; slideIndex++) {
	...
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", { class: this.getCssClassMap() }, h("div", { class: "carousel__container" }, h("div", {
			class: "carousel__arrow carousel__arrow--left",
			onClick: () => this.handleSlideChange("prev")
		}, h("slot", { name: "arrow-left" })), this.slidesArray.map((element) => h("div", {
			class: "carousel__slide",
			style: { transform: this.setTransformValue() }
		}, h("div", { innerHTML: element.outerHTML }))), h("div", {
			class: "carousel__arrow carousel__arrow--right",
			onClick: () => this.handleSlideChange("next")
		}, h("slot", { name: "arrow-right" }))), h("ul", { class: "carousel__indicators" }, Array.from(Array(this.slidesArray.length).keys()).map((index) => h("li", {
			key: index,
...
//# sourceMappingURL=scale-carousel.entry-BV2vmSbu.js.map