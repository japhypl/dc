...
//#region node_modules/@telekom/scale-components/dist/esm/scale-icon-action-menu_7.entry.js
...
var ActionMenu = class {
	...
	render() {
		...
		return h(Host, null, h("svg", Object.assign({
			...
			d: "M14.335 16.055a1.25 1.25 0 01.128 2.494l-.128.006H4.75a1.25 1.25 0 01-.128-2.494l.128-.006h9.585zM19.25 10.5a1.25 1.25 0 01.128 2.494L19.25 13H4.75a1.25 1.25 0 01-.128-2.494l.128-.006h14.5zm0-5.5a1.25 1.25 0 01.128 2.494l-.128.006H4.75a1.25 1.25 0 01-.128-2.494L4.75 5h14.5z",
			"fill-rule": "evenodd"
		})) : h("g", null, h("path", {
			d: "M14.75 16.5c.4 0 .75.35.75.75a.772.772 0 01-.651.743L14.75 18H4.25c-.4 0-.75-.35-.75-.75 0-.367.294-.691.651-.743l.099-.007h10.5zm5-5.5c.4 0 .75.35.75.75a.772.772 0 01-.651.743l-.099.007H4.25c-.4 0-.75-.35-.75-.75 0-.367.294-.691.651-.743L4.25 11h15.5zm0-5.5c.4 0 .75.35.75.75a.772.772 0 01-.651.743L19.75 7H4.25c-.4 0-.75-.35-.75-.75 0-.367.294-.691.651-.743L4.25 5.5h15.5z",
			"fill-rule": "evenodd"
		})))));
	}
	get hostElement() {
...
var TelekomHeader = class {
	...
	onScroll() {
		...
		this.scrolledBack = this.pageYOffset !== window.pageYOffset && window.pageYOffset <= 0;
	...
	render() {
		...
		}, h("header", { part: classnames("base", this.type, {
			scrolled: this.type !== "subtle" && this.scrolled,
			"scrolled-back": this.type !== "subtle" && this.scrolledBack
		}) }, h("div", { part: "fixed-wrapper" }, h("div", { part: "container" }, h("slot", { name: "logo" }, h("scale-logo", {
			part: "app-logo",
			variant: "white",
			href: this.logoHref,
		...
		})), h("div", { part: "body" }, h("div", { part: "top-bar" }, this.appName ? h("div", { part: "top-app-name" }, this.appNameLink ? h("a", {
			part: "app-name-text",
			onClick: this.appNameClick,
			href: this.appNameLink
		}, this.appName) : h("span", { part: "app-name-text" }, this.appName)) : null, h("div", { part: "top-body" }, h("nav", {
			part: "meta-nav-external",
			"aria-label": this.metaNavExternalAriaLabel
		}, h("slot", { name: "meta-nav-external" })), h("nav", {
			part: "meta-nav",
			"aria-label": this.metaNavAriaLabel
		}, h("slot", { name: "meta-nav" })), h("nav", {
			part: "lang-switcher",
			"aria-label": this.langSwitcherAriaLabel
		}, h("slot", { name: "lang-switcher" })))), h("div", { part: "bottom-bar" }, this.appName ? h("div", { part: "bottom-app-name" }, this.appNameLink ? h("a", {
			part: "app-name-text",
			onClick: this.appNameClick,
			href: this.appNameLink
		}, this.appName) : h("span", { part: "app-name-text" }, this.appName)) : null, h("div", { part: "bottom-body" }, h("nav", {
			part: "main-nav",
			"aria-label": this.mainNavAriaLabel
		}, h("slot", { name: "main-nav" })), h("slot", { name: "functions" }))))))));
	}
	get hostElement() {
		return getElement(this);
	}
...
var TelekomMegaMenu = class {
	...
	connectedCallback() {
		if (this.hostElement.children.length > 4) this.childrenTooMany = true;
	}
	render() {
		return h(Host, {
			class: "scale-telekom-mega-menu",
			"children-too-many": this.childrenTooMany
		}, h("div", { class: "scale-telekom-mega-menu-container" }, h("slot", null)));
	}
	get hostElement() {
		return getElement(this);
	}
...
var TelekomMegaMenuColumn = class {
	...
	connectedCallback() {
		if (this.hostElement.querySelector("a[slot=\"heading\"]")) this.headingHasLink = true;
	}
	render() {
		return h(Host, null, h("div", { part: classnames({
			...
			part: "icon",
			"aria-hidden": "true"
		}, h("slot", { name: "icon" })), h("div", { part: "body" }, h("div", {
			part: "heading",
			role: "heading",
			"aria-level": this.headingLevel
		}, h("slot", { name: "heading" }), h("scale-icon-navigation-right", {
...
var TelekomMobileMenu = class {
	...
	handleSetMenuItemActive(e) {
		...
		if (e.target.parentElement.tagName === "SCALE-TELEKOM-MOBILE-MENU-ITEM") e.target.parentElement.setAttribute("active", "");
	}
	...
	connectedCallback() {
		this.setLevelAttributeForAllItems();
		this.currentLevel = this.activeItem ? String(+this.activeItem.getAttribute("level")) : "0";
		Array.from(this.menuItems).forEach((element) => {
			element.setAttribute("current-level", this.currentLevel);
		});
	}
	...
	get activeItem() {
		return Array.from(this.menuItems).find((element) => element.hasAttribute("active") || element.active);
	}
	get openItems() {
		return Array.from(this.menuItems).filter((element) => element.hasAttribute("open") || element.open);
	}
	render() {
		return h(Host, null, h("div", { part: "base" }, h("nav", { part: "nav" }, +this.currentLevel > 0 ? h("button", {
			part: "back-button",
			onClick: () => {
				this.back();
			}
...
var TelekomMobileMenuItem = class {
	...
	toggleChildrenVisibility(show) {
		this.children.forEach((element) => {
			show && element.getAttribute("level") === String(+this.level + 1) ? element.removeAttribute("hidden") : element.setAttribute("hidden", "");
		});
	}
	get children() {
		return this.hostElement.querySelectorAll("scale-telekom-mobile-menu-item");
	...
	render() {
		...
		}) }, h("div", { part: classnames("header", { hidden: !!this.openChildren.length }) }, h("slot", null), h("div", { part: "icon-right-container" }, !!this.children.length && !this.open && h("scale-icon-navigation-right", {
			size: 20,
...
var TelekomNavList = class {
	...
	handleScaleExpanded(event) {
		if (event.detail.expanded) this.closeExpandedFlyoutSiblings(event.target);
	}
	closeExpandedFlyoutSiblings(target) {
		[...this.hostElement.children].filter((x) => !x.contains(target)).forEach((item) => {
			const flyout = item.querySelector("scale-telekom-nav-flyout");
			if (isDirectChild(item, flyout) && flyout.expanded) flyout.expanded = false;
		});
	}
	connectedCallback() {
...
//#endregion
export { ActionMenu as scale_icon_action_menu, TelekomHeader as scale_telekom_header, TelekomMegaMenu as scale_telekom_mega_menu, TelekomMegaMenuColumn as scale_telekom_mega_menu_column, TelekomMobileMenu as scale_telekom_mobile_menu, TelekomMobileMenuItem as scale_telekom_mobile_menu_item, TelekomNavList as scale_telekom_nav_list };

//# sourceMappingURL=scale-icon-action-menu_7.entry-DZNCt9Cb.js.map