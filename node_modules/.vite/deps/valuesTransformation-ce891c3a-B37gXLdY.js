//#region node_modules/@telekom/scale-components/dist/esm/valuesTransformation-ce891c3a.js
function createCssString(setProp) {
	const values = setProp.breakpointValues;
	if (!isBreakpointValuesEmpty(values)) {
		const valuesObj = transformBreakpointValuesData(fillEmptyBreakpointValues(values));
		return Object.entries(valuesObj).map(([key, value]) => `--${setProp.propName}-${key}:${value}`).join(";") + ";";
	}
}
...
function fillEmptyBreakpointValues(values) {
	...
	for (let i = 1; i < filledArray.length; i++) if (!filledArray[i].value) filledArray[i].value = filledArray[i - 1].value;
	return filledArray;
}
...
//#endregion
export { createCssString as n, createBreakpointValuedProp as t };

//# sourceMappingURL=valuesTransformation-ce891c3a-B37gXLdY.js.map