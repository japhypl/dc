...
//#region node_modules/@telekom/scale-components/dist/esm/scale-segment.entry.js
...
var Segment = class {
	constructor(hostRef) {
		...
		this.handleClick = (event) => {
			if (this.selectedIndex === this.position) return;
	...
	getAriaDescriptionTranslation() {
		...
		return this.ariaDescriptionTranslation.replace(/\$position/g, `${this.position}`).replace(/\$selected/g, `${replaceSelected}`);
	}
	handleIcon() {
	...
	render() {
		...
		}, h("div", { class: "segment--mask" }, !this.iconOnly && h("div", { class: "success-icon-container" }, h("scale-icon-action-checkmark", {
			size: this.size === "small" ? 14 : this.size === "medium" ? 16 : 20,
			class: "scale-icon-action-checkmark",
			"aria-hidden": "true",
			selected: true
		})), h("div", { class: "icon-container" }, h("slot", { name: "segment-icon" })), h("slot", null))));
	}
	getBasePartMap() {
	...
	static get watchers() {
		return { "selected": ["selectionChanged"] };
	}
};
...
//# sourceMappingURL=scale-segment.entry-ChJmyNQq.js.map