...
//#region node_modules/@telekom/scale-components/dist/esm/scale-modal.entry.js
/**
...
function queryShadowRoot(root, skipNode, isMatch, maxDepth = 20, depth = 0) {
	...
	for (const $child of children) {
		...
		if ($child.shadowRoot != null) matches.push(...queryShadowRoot($child.shadowRoot, skipNode, isMatch, maxDepth, depth + 1));
		else if ($child.tagName === "SLOT") matches.push(...traverseSlot($child));
		else matches.push(...queryShadowRoot($child, skipNode, isMatch, maxDepth, depth + 1));
	}
	return matches;
}
/**
...
function isFocusable($elem) {
	if ($elem.getAttribute("tabindex") === "-1" || isHidden($elem) || isDisabled($elem)) return false;
...
var keyframeDefaults = { easing: "cubic-bezier(0.390, 0.575, 0.565, 1.000)" };
var KEYFRAMES = {
	fadeIn: [Object.assign(Object.assign({ offset: 0 }, keyframeDefaults), { opacity: 0 }), Object.assign(Object.assign({ offset: 1 }, keyframeDefaults), { opacity: 1 })],
	fadeOut: [Object.assign(Object.assign({ offset: 0 }, keyframeDefaults), { opacity: 1 }), Object.assign(Object.assign({ offset: 1 }, keyframeDefaults), { opacity: 0 })],
	fadeInTop: [Object.assign(Object.assign({ offset: 0 }, keyframeDefaults), {
		opacity: 0,
		/**
		...
		*/
		top: "-3rem"
	}), Object.assign(Object.assign({ offset: 1 }, keyframeDefaults), {
		opacity: 1,
		top: 0
	})]
};
...
var Modal = class {
	constructor(hostRef) {
		...
		this.handleKeypress = (event) => {
			...
			if (event.key === "Escape") this.emitBeforeClose("ESCAPE_KEY");
		};
		this.handleTopFocus = () => {
	...
	*/
	componentWillRender() {
		...
		if (bodySlot != null) this.hasBody = bodySlot.assignedElements().length > 0;
	}
	emitBeforeClose(trigger) {
		if (!emitEvent(this, "scaleBeforeClose", { trigger }).some((event) => event.defaultPrevented)) this.opened = false;
	}
	componentDidLoad() {
	...
	openedChanged(newValue) {
		if (newValue === true) {
			...
			this.close();
			if (this.allowInjectingStyleToBody) document.body.style.setProperty("overflow", this.bodyOverflowValue);
		}
	}
	open() {
		this.isOpen = true;
		try {
			...
			animateTo(this.modalContainer, KEYFRAMES.fadeIn, { duration: this.duration }).addEventListener("finish", () => {
				this.attemptFocus(this.getFirstFocusableElement());
	...
	close() {
		try {
			animateTo(this.modalContainer, KEYFRAMES.fadeOut, { duration: this.duration }).addEventListener("finish", () => {
				this.isOpen = false;
	...
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", {
			...
			part: "heading"
		}, this.heading), !this.omitCloseButton && h("button", {
		...
		}, h("slot", { name: "close-icon" }, h("scale-icon-action-circle-close", { decorative: true })))), h("div", {
			...
			part: classnames("actions", `align-${this.alignActions}`, this.hasActionsSlot && "has-actions", this.hasScroll && "has-scroll")
		}, h("slot", { name: "action" }))), h("div", {
			"data-focus-trap-edge": true,
...
//# sourceMappingURL=scale-modal.entry-DUZ-Ku-q.js.map