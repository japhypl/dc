...
import { t as findRootNode } from "./menu-utils-ee192675-DzOday92.js";
import { t as classnames } from "./index-713f92a5-R2Q0UnM3.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-app-header.entry.js
...
var Header = class {
	...
	handleCloseMenu() {
		if (this.mobileMenu) this.mobileMenuToggle.focus();
		this.mobileMenu = false;
	}
	handleCloseUserMenu() {
		if (this.userMenuToggle) this.userMenuToggle.focus();
		this.userMenu = false;
	}
	handleOpenUserMenu() {
		this.userMenu = true;
	...
	componentWillRender() {
		...
		if (this.isMobileMenuVisible !== false) statusNote({
			tag: "deprecated",
	...
	handleMobileMenu(event) {
		...
		if (event && "key" in event) {
			...
			if (event.key === "Escape" && !this.mobileMenu) return;
			if (event.key === "Enter" && this.mobileMenu) return;
		}
		this.userMenuMobile = false;
	...
	menuMain() {
		...
		return h("ul", {
			...
			onKeyDown: (e) => {
				if (e.key === "Escape") this.visibleMegaMenu = "";
			}
		}, this.hasSlotMenuMain ? h("slot", { name: "menu-main" }) : readData(this.mainNavigation).map((item) => h("scale-nav-main", {
			href: item.href,
			...
			clickLink: (event) => {
				if (item.href) this.visibleMegaMenu = "";
				if (typeof item.onClick === "function") item.onClick(event);
				this.visibleMegaMenu = item.children ? item.id : null;
			},
			name: item.name
		}, item.children && item.children.length > 0 && h("app-mega-menu", {
	...
	menuIcon() {
		const { defaultName, openedName } = readData(this.iconNavigation).find(({ id }) => id === "menu") || {
			defaultName: "Menu",
			openedName: "Close"
		};
		const { shortName = "Login", badge, badgeLabel } = readData(this.userNavigation).find(({ type }) => type === "userInfo") || { shortName: "Login" };
		return h("ul", { class: "meta-navigation" }, this.hasSlotMenuIcon ? h("slot", { name: "menu-icon" }) : readData(this.iconNavigation).filter(({ id }) => id !== "menu").map((item) => h("scale-nav-icon", {
			icon: item.icon,
			href: item.href,
			badge: item.badge,
		...
		}, item.name)), readData(this.userNavigation).length > 0 && h("span", null, h("span", {
			class: "header__user-menu--desktop",
			"aria-label": this.userMenuAriaLabel
		}, h("scale-menu-flyout", null, h("scale-nav-icon", {
			slot: "trigger",
		...
		}, shortName), h("scale-menu-flyout-list", null, h("app-navigation-user-menu", {
			hide: () => {
			...
			icon: "user-file-user",
			refMobileUserMenuToggle: (el) => this.userMenuMobileToggle = el,
			clickLink: () => {
				this.mobileMenu = false;
		...
		}, shortName))), (readData(this.mainNavigation).length > 0 || this.hasSlotMenuMobile) && h("scale-nav-icon", {
	...
	menuSector() {
		return h("ul", { class: "sector-navigation" }, this.hasSlotMenuSector ? h("slot", { name: "menu-sector" }) : this.portalName ? h("li", { class: "sector-navigation__portal-name" }, this.portalName) : readData(this.sectorNavigation).map((item) => h("scale-nav-segment", {
	...
	menuAddon() {
		return h("ul", { class: "addon-navigation" }, this.hasSlotMenuAddon ? h("slot", { name: "menu-addon" }) : readData(this.addonNavigation).map((item) => h("scale-nav-segment", {
			href: item.href,
			onClick: (event) => {
				if (typeof item.onClick === "function") item.onClick(event);
			},
	...
	render() {
		return h(Host, null, h("header", { class: "header__container" }, h("div", { class: this.getCssClassMap() }, h("div", { class: "header__brand" }, h("span", { class: "header__brand-before" }), h("span", { class: "header__brand-after" }), h("div", { class: "header__brand-content" }, h("div", { class: "header__brand-branding" }, this.hasSlotLogo ? h("slot", { name: "logo" }) : h("scale-logo", {
			transparent: true,
			href: this.logoHref,
		...
		})), h("div", { class: "header__brand-sector" }, this.menuSector()), h("div", { class: "header__brand-meta" }, this.menuAddon()))), h("nav", {
			class: "header__nav",
			"aria-label": "top"
		}, h("span", { class: "header__nav-before" }), h("span", { class: "header__nav-after" }), h("div", { class: "header__nav-content" }, h("div", { class: "header__nav-logo" }, this.hasSlotLogoInverse ? h("slot", { name: "logo-inverse" }) : h("scale-logo", {
			transparent: true,
			language: "",
			href: this.logoHref,
			logoTitle: this.logoTitle,
			...
			logoAriaDescribedBy: this.logoAriaDescribedBy,
			logoAriaHidden: !this.scrolled
		})), h("div", { class: "header__nav-menu-wrapper" }, h("div", { class: "header__nav-menu-main" }, this.menuMain()), h("div", { class: "header__nav-menu-icon" }, this.menuIcon())))), h("nav", {
			class: `header__nav__mobile-menu${this.mobileMenu ? " header__nav__mobile-menu--opened" : ""}`,
			"aria-label": "main"
		}, this.hasSlotMenuMobile ? h("slot", { name: "menu-mobile" }) : h("div", null, h("app-navigation-sector-mobile", {
			...
			class: `header__nav__mobile-menu${this.userMenuMobile ? " header__nav__mobile-menu--opened" : ""}`,
			"aria-label": "main"
		}, h("div", null, this.userMenuMobile && h("app-navigation-user-menu", {
			hide: () => {
				if (!this.userMenuMobile) return;
				this.userMenuMobile = false;
...
//#endregion
export { Header as scale_app_header };

//# sourceMappingURL=scale-app-header.entry-Bd4K9JY8.js.map