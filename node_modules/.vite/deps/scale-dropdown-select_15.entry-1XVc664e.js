...
import { n as emitEvent, o as isPseudoClassSupported, r as generateUniqueId } from "./utils-c4af5b47-5j7RUTeV.js";
import { n as computePosition } from "./floating-ui.dom.esm-43ac86e1-BRqXGRTG.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-dropdown-select_15.entry.js
...
var isElementDisabled = (x) => {
	return typeof x.disable === "boolean";
};
var readDisabled = (element) => {
	const attr = element.getAttribute("disabled");
	return attr !== null && `${attr}` !== "false" || (isElementDisabled(element) ? element.disable : false);
};
var readOptions = (hostElement) => {
	return Array.from(hostElement.children).filter((x) => x.tagName !== "INPUT" && x.hidden === false).map((x) => {
		var _a;
		return {
...
function getActionFromKey(event, open) {
	...
	if (!open && [
		...
		"Enter",
		" "
	].includes(key)) return Actions["Open"];
	if (key === "Home") return Actions["First"];
	if (key === "End") return Actions["Last"];
	if (["Backspace", "Clear"].includes(key) || key.length === 1 && key !== " " && !altKey && !ctrlKey && !metaKey) return Actions["Type"];
	if (!open) return;
	if (key === "ArrowUp" && altKey) return Actions["CloseSelect"];
	if (key === "ArrowDown" && !altKey) return Actions["Next"];
	switch (key) {
		case "ArrowUp": return Actions["Previous"];
		case "PageUp": return Actions["PageUp"];
		case "PageDown": return Actions["PageDown"];
		case "Escape": return Actions["Close"];
		case "Enter": return Actions["CloseSelect"];
		case " ": return Actions["CloseSelect"];
	}
}
function jumpToIndex(from, action, options) {
	...
	const findNearestEnabled = (current, step) => {
		...
		do {
			...
			if (nextOption === void 0) break;
		} while (nextOption === null || nextOption === void 0 ? void 0 : nextOption.disabled);
		return nextOption ? nextIndex : current;
	};
	let nearest;
	switch (action) {
		case Actions["First"]: return ((_a = options[0]) === null || _a === void 0 ? void 0 : _a.disabled) ? findNearestEnabled(-1, 1) : 0;
		case Actions["Last"]:
			nearest = findNearestEnabled(options.length, -1);
			return nearest === options.length ? -1 : nearest;
		case Actions["Previous"]:
			nearest = findNearestEnabled(from, from === -1 ? 1 : -1);
			return nearest === options.length ? -1 : nearest;
		case Actions["Next"]: return findNearestEnabled(from, 1);
		case Actions["PageUp"]: return findNearestEnabled(Math.max(from - JUMP_SIZE, -1), 1);
		case Actions["PageDown"]:
			nearest = findNearestEnabled(Math.min(from + JUMP_SIZE, options.length), -1);
			return nearest === options.length ? -1 : nearest;
		default: return from;
	}
}
function matchEnabledOptions(options = [], filter) {
	return options.filter((option) => !option.disabled && option.label.toLowerCase().indexOf(filter.toLowerCase()) === 0);
}
function getIndexByChar(values, filter, startIndex = 0) {
	const sortedOptions = [...values.slice(startIndex), ...values.slice(0, startIndex)];
	const firstHit = matchEnabledOptions(sortedOptions, filter)[0];
	const allMatchingChars = (array) => array.every((char) => char === array[0]);
	if (firstHit) return values.indexOf(firstHit);
...
function keepInView(activeElement, scrollParent) {
	...
	if (offsetTop + offsetHeight > scrollTop + parentOffsetHeight) return scrollParent.scrollTop = offsetTop + offsetHeight - parentOffsetHeight;
	if (isAboveParent) return scrollParent.scrollTop = offsetTop;
}
...
var DropdownSelect = class {
	constructor(hostRef) {
		...
		this.handleKeyDown = (event) => {
			...
			switch (action) {
				case Actions["Last"]:
				case Actions["First"]: this.setOpen(true);
				case Actions["Next"]:
				case Actions["Previous"]:
				case Actions["PageUp"]:
				case Actions["PageDown"]:
				...
				case Actions["CloseSelect"]:
					event.preventDefault();
					if ((_a = options[this.currentIndex]) === null || _a === void 0 ? void 0 : _a.disabled) return;
					if (this.currentIndex !== -1) this.selectOption(this.currentIndex);
				case Actions["Close"]:
					event.preventDefault();
					return this.setOpen(false);
				case Actions["Type"]: return this.buildQueryString(key);
				case Actions["Open"]:
					event.preventDefault();
		...
		this.handleClick = () => {
			if (activeDropdown && activeDropdown !== this) activeDropdown.setOpen(false);
			this.setOpen(!this.open);
			activeDropdown = this;
			const indexOfValue = readOptions(this.hostElement).findIndex(({ value }) => value === this.value);
			if (indexOfValue > -1) setTimeout(() => {
	...
	componentDidRender() {
		if (!this.open) return;
		if (this.floatingStrategy === "fixed") this.listboxPadEl.style.width = `${this.comboEl.getBoundingClientRect().width}px`;
	...
	handleOptionChange(index) {
		this.currentIndex = index;
		if (index > -1) this.bringIntoView(index);
	}
	bringIntoView(index) {
		const options = this.scrollContainer.querySelectorAll("[role=option]");
		if (hasOverflow(this.scrollContainer)) keepInView(options[index], this.scrollContainer);
		if (!isInView(options[index])) options[index].scrollIntoView({
			behavior: "smooth",
			block: "nearest"
		});
	}
	...
	handleOptionClick(event, index) {
		event.stopPropagation();
		if (readOptions(this.hostElement)[index].disabled) return;
	...
	buildQueryString(char) {
		...
		if (queryIndex >= 0) this.handleOptionChange(queryIndex);
		else {
	...
	render() {
		...
		const ariaDescribedByAttr = { "aria-describedBy": this.helperText ? helperTextId : this.ariaDetailsId };
		return h(Host, null, h("div", { part: this.getBasePartMap() }, h("div", { part: "combobox-container" }, h("label", {
			id: `${this.comboboxId}-label`,
			part: "label"
		}, this.label), h("div", Object.assign({
		...
		}, this.open ? { "aria-activedescendant": (readOptions(this.hostElement)[this.currentIndex] || {}).value } : {}, ariaDescribedByAttr, this.invalid ? { "aria-invalid": "true" } : {}), h("span", { part: "combobox-value" }, hasEmptyValueElement ? "" : ValueElement)), h("div", {
			part: "listbox-pad",
			ref: (el) => this.listboxPadEl = el
		}, h("div", {
			...
			"aria-labelledby": `${this.comboboxId}-label`,
			tabindex: "-1"
		}, readOptions(this.hostElement).map(({ value, disabled, ItemElement }, index) => h("div", Object.assign({
			role: "option",
		...
		}, value === this.value ? { "aria-selected": "true" } : {}, disabled ? { "aria-disabled": "true" } : {}), ItemElement, value === this.value ? h("div", null, h("scale-icon-action-checkmark", { size: 16 }), h("span", { class: "sr-only" }, this.ariaLabelSelected)) : null))))), this.allowClear && this.value ? h("scale-icon-action-close", {
			"accessibility-title": "close",
			part: "clear",
			onClick: this.handleClearClick
		}) : h("div", { part: "icon" }, this.open ? h("scale-icon-navigation-collapse-up", {
			decorative: true,
			size: DEFAULT_ICON_SIZE$1
		}) : h("scale-icon-navigation-collapse-down", {
			decorative: true,
			size: DEFAULT_ICON_SIZE$1
		}))), this.helperText && h("scale-helper-text", {
...
var iconCss$6 = "scale-icon{--display:inline-flex;--transition:all var(--telekom-motion-duration-transition)\n    var(--telekom-motion-easing-standard);display:var(--display)}scale-icon path{transition:var(--transition)}@media screen and (forced-colors: active), (-ms-high-contrast: active){scale-icon svg,.scale-icon{color:white;stroke:white}}";
var ActionEdit = class {
	...
	connectedCallback() {
		if (!this.hostElement.hasAttribute("styles")) this.hostElement.style.display = "inline-flex";
	}
	render() {
		...
		return h(Host, null, h("svg", Object.assign({
			...
			height: this.size,
			viewBox: "0 0 24 24"
		}, ariaHidden, focusable), this.accessibilityTitle && h("title", null, this.accessibilityTitle), h("g", { fill: this.fill === "currentColor" ? this.color : this.fill }, this.selected ? h("g", null, h("path", {
			d: "M21.25 19.5a.75.75 0 01.102 1.493L21.25 21H8.045l1.05-1.5H21.25zM3.355 17.15l3.14 2.205L3 20.995l.35-3.84.005-.005zM15.46 1.29v.02a1.915 1.915 0 011.656 2.862l-.086.138-9.675 13.815-3.14-2.2 9.67-13.815a1.92 1.92 0 011.405-.813l.17-.007z",
			"fill-rule": "evenodd"
		})) : h("g", null, h("path", {
			d: "M21.25 19.5c.4 0 .75.35.75.75a.772.772 0 01-.651.743L21.25 21H8.05l1.05-1.5h12.15zm-17.9-2.35l3.15 2.2L3 21l.35-3.85zM13.9 2.1c.522-.76 1.677-1.024 2.52-.534l.13.084.136.1a1.88 1.88 0 01.452 2.413l-.088.137-9.7 13.8-3.15-2.2 9.7-13.8z",
			"fill-rule": "evenodd"
		})))));
	}
	get hostElement() {
...
ActionEdit.style = iconCss$6;
var iconCss$5 = "scale-icon{--display:inline-flex;--transition:all var(--telekom-motion-duration-transition)\n    var(--telekom-motion-easing-standard);display:var(--display)}scale-icon path{transition:var(--transition)}@media screen and (forced-colors: active), (-ms-high-contrast: active){scale-icon svg,.scale-icon{color:white;stroke:white}}";
...
ActionHidePassword.style = iconCss$5;
...
var ActionSort = class {
	...
	render() {
		...
		return h(Host, null, h("svg", Object.assign({
			...
			d: "M17.25 3.5c.647 0 1.18.492 1.244 1.122l.006.128V14h3.17l-4.42 6.75L12.83 14H16V4.75c0-.69.56-1.25 1.25-1.25zm-10.5-.25L11.17 10H8v9.25a1.25 1.25 0 01-2.494.128L5.5 19.25V10H2.33l4.42-6.75z",
			"fill-rule": "evenodd"
		})) : h("g", null, h("path", {
			d: "M17.25 4c.367 0 .691.294.743.651L18 4.75V14h3.65l-4.4 6.75-4.4-6.75h3.65V4.75c0-.4.35-.75.75-.75zm-10.5-.75l4.4 6.75H7.5v9.25c0 .4-.35.75-.75.75a.772.772 0 01-.743-.651L6 19.25V10H2.35l4.4-6.75z",
			"fill-rule": "evenodd"
		})))));
	}
	get hostElement() {
...
var MenuFlyoutItem = class {
	...
	async triggerEvent(event, closeOnSelect = true) {
		const { key } = event;
		if (this.disabled) return;
		if (key === "ArrowRight" && !this.hasSlotSublist) return;
		if (this.hasSlotSublist) {
			const sublist = this.hostElement.querySelector("[slot=\"sublist\"]");
			if (sublist.hasAttribute("opened")) sublist.removeAttribute("opened");
			else this.openSublist();
			return;
		}
		const detail = {
	...
	render() {
		...
		return h(Host, {
			...
			part: "prefix",
			class: "menu-flyout-item__prefix"
		}, this.checkable == null ? h("slot", { name: "prefix" }) : h("scale-icon-action-checkmark", {
			class: "menu-flyout-item__check",
			size: 16
		})), h("span", {
			part: "label",
			class: "menu-flyout-item__label"
		}, h("slot", null)), h("span", {
			part: "suffix",
			class: "menu-flyout-item__suffix"
		}, this.cascade ? h("scale-icon-navigation-right", {
			class: "menu-flyout-item__cascade",
			size: 16
		}) : h("slot", { name: "suffix" }))), h("slot", { name: "sublist" }));
	}
	get hostElement() {
		return getElement(this);
	}
...
var Pagination = class {
	...
	componentWillUpdate() {}
	componentDidRender() {
		if (this.hideBorders !== false) statusNote({
			tag: "deprecated",
	...
	render() {
		...
		}, h("span", null, start, "-", end), " ", "/ ", total), h("div", { class: `${name}__button-wrapper` }, h("div", {
			part: "info",
			class: `${name}__info`,
			style: { width: `${this.maxWidth}px` }
		}, h("span", null, start, "-", end), " ", "/ ", total), h("button", {
			class: `${name}__first-prompt`,
			part: "first-prompt",
			disabled: isAtStart,
			onClick: () => this.goFirstPage(),
			"aria-label": this.ariaLabelFirstPage
		}, h("scale-icon-navigation-double-left", {
		...
		}, h("scale-icon-navigation-double-right", {
			size: DEFAULT_ICON_SIZE,
			decorative: true
		})))));
	}
	...
	static get watchers() {
		return { "totalElements": ["calculateWidth"] };
	}
};
...
var ProgressBar = class {
  ...
  `;
	...
	componentWillLoad() {
		if (this.progressBarId == null) this.progressBarId = "progress-bar-" + i$1++;
		if (this.disabled) this.showStatus = false;
	}
	componentWillUpdate() {}
	disconnectedCallback() {}
	componentDidRender() {
		if (this.customColor !== void 0) statusNote({
			tag: "deprecated",
	...
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("style", null, this.transitions(this.percentage, this.percentageStart)), h("div", {
			part: this.getBasePartMap(),
			class: this.getCssClassMap()
		}, h("div", { class: "progress-bar__top-container" }, !!this.label ? h("label", {
			part: "label",
			class: "progress-bar__label",
			htmlFor: this.progressBarId
		}, this.label) : h("span", null, " "), !!this.showStatus && !this.hasError && this.percentage <= 100 && this.percentage !== 100 && h("div", {
			part: "status",
			class: "progress-bar__status",
			"aria-hidden": "true"
		}, this.percentage, "%"), this.hasError ? h("div", { class: "progress-bar__icon" }, h("scale-icon-alert-error", { size: ICON_SIZE })) : this.percentage >= 100 ? h("div", { class: "progress-bar__icon" }, h("scale-icon-action-success", { size: ICON_SIZE })) : null), h("div", {
			part: "wrapper",
		...
		}, this.percentage > 0 && h("div", {
			part: "inner",
			class: "progress-bar__inner",
			style: this.progressStyle()
		})), h("slot", { name: "icon" }))), !!this.statusDescription && h("div", {
			part: "status-description",
			class: "progress-bar__status-description",
			role: "alert"
		}, this.statusDescription), !this.mute && h("span", {
			"aria-live": "polite",
			class: "progress-bar__aria-live"
		}, this.percentage !== Math.round(this.percentage / 10) * 10 ? `${Math.round(this.percentage / 10) * 10}%` : null));
	}
	getBasePartMap() {
		return this.getCssOrBasePartMap("basePart");
	}
...
var Switch = class {
	...
	componentWillLoad() {
		if (this.inputId == null) this.inputId = "switch-" + i++;
	}
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", { class: this.getCssClassMap() }, h("label", {
			...
			"aria-hidden": "true"
		}, h("span", { class: "switch__thumb" }, h("scale-icon-action-checkmark", {
			size: 12,
			decorative: true,
			selected: true
		})), h("span", { class: "switch__io-text" }, h("span", null, this.checked ? "I" : "0"))), h("span", {
			class: "switch__toggle--overlay",
			"aria-hidden": "true"
...
var Tag = class {
	...
	render() {
		...
		return h(Host, null, this.styles && h("style", null, this.styles), h(Element, Object.assign({
			part: this.getBasePartMap(),
			class: this.getCssClassMap()
		}, linkProps), h("slot", null), this.dismissable && h("button", {
			part: "button-dismissable",
			disabled: this.disabled,
			"aria-label": this.dismissText,
			onClick: this.handleClose
		}, h("scale-icon-action-close", {
...
var TextField = class {
	...
	componentWillLoad() {
		if (this.inputId == null) this.inputId = "input-text-field-" + this.internalId;
	}
	componentDidRender() {
		const value = this.value == null ? "" : this.value.toString();
		const input = this.hostElement.querySelector("input");
		if (this.experimentalControlled && input.value.toString() !== value) input.value = value;
	...
	emitChange() {
		emitEvent(this, "scaleChange", { value: this.value == null ? this.value : this.value.toString() });
	}
	render() {
		const ariaInvalidAttr = this.status === "error" || this.invalid ? { "aria-invalid": "true" } : {};
		const helperTextId = `helper-message-${this.internalId}`;
		const ariaDescribedByAttr = { "aria-describedBy": this.helperText && helperTextId || this.ariaDetailedId };
		const ariaDetailsAttr = { "aria-details": this.ariaDetailedId };
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", { class: this.getCssClassMap() }, h("label", {
			class: "text-field__label",
			htmlFor: this.inputId
		}, this.label), h("input", Object.assign({
			type: this.type,
			inputMode: this.inputModeType,
			class: "text-field__control",
			value: this.value
		}, !!this.name ? { name: this.name } : {}, !!this.inputAutofocus ? { autofocus: "true" } : {}, {
		...
		}, ariaInvalidAttr, this.helperText || this.ariaDetailedId ? ariaDescribedByAttr : {}, this.helperText && this.ariaDetailedId ? ariaDetailsAttr : {}, [
			"number",
			"date",
			"month",
			"week",
			"time",
			"datetime-local"
		].includes(this.type) ? { step: this.step } : {})), (!!this.helperText || !!this.counter) && h("div", {
			...
			helperText: this.helperText,
			variant: this.invalid ? "danger" : this.variant
		}), this.counter && h("div", { class: "text-field__counter" }, !!this.value ? String(this.value).length : 0, " /", " ", this.maxLength))));
	}
	getCssClassMap() {
		const animated = this.value != null && this.value !== "" || [
			"date",
...
//#endregion
export { DropdownSelect as scale_dropdown_select, DropdownSelectItem as scale_dropdown_select_item, ActionEdit as scale_icon_action_edit, ActionHidePassword as scale_icon_action_hide_password, ActionRemove as scale_icon_action_remove, ActionSort as scale_icon_action_sort, ContentSortIndicatorDown as scale_icon_content_sort_indicator_down, ContentSortIndicatorUp as scale_icon_content_sort_indicator_up, ServiceSettings as scale_icon_service_settings, MenuFlyoutItem as scale_menu_flyout_item, Pagination as scale_pagination, ProgressBar as scale_progress_bar, Switch as scale_switch, Tag as scale_tag, TextField as scale_text_field };

//# sourceMappingURL=scale-dropdown-select_15.entry-1XVc664e.js.map