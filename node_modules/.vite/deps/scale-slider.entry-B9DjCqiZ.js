import { a as createEvent, l as registerInstance, n as Host, o as getElement, s as h, t as Fragment } from "./index-6d95a4bc-DlO5twzZ.js";
...
//#region node_modules/@telekom/scale-components/dist/esm/scale-slider.entry.js
...
var Slider = class {
	constructor(hostRef) {
		...
		this.onKeyDown = (event) => {
			let steps = 0;
			this.setActiveRangeThumbFromEvent(event);
			if (["ArrowRight", "ArrowLeft"].includes(event.key)) steps = event.key === "ArrowRight" ? this.step : -this.step;
			if (["ArrowUp", "ArrowDown"].includes(event.key)) steps = event.key === "ArrowUp" ? this.step * 10 : -this.step * 10;
		...
		this.onDragging = (event) => {
			...
			const nextValue = (this.handleTouchEvent(event).clientX - offsetLeft) / this.sliderTrack.offsetWidth * 100 * (this.max - this.min) / 100 + this.min;
		...
		*/
		this.getKeyFor = (propName, thumb) => {
			var _a;
			if (this.range) return `${propName}${(_a = this.activeRangeThumb) !== null && _a !== void 0 ? _a : thumb}`;
			return propName;
		};
		this.getTextValue = () => {
			var _a, _b, _c, _d;
		...
		this.clamp = (val) => {
			...
			let max = this.max;
			if (this.range) {
				if (this.activeRangeThumb === "From") max = Math.min(this.valueTo, this.max);
				else if (this.activeRangeThumb === "To") min = Math.max(this.valueFrom, this.min);
			}
	...
	componentWillLoad() {
		if (this.sliderId == null) this.sliderId = "slider-" + this.internalId;
		if (this.range) {
	...
	componentDidLoad() {
		...
		if (this.thumbLarge !== void 0) statusNote({
			tag: "deprecated",
			message: `Property "thumbLarge" is deprecated.`,
			type: "warn",
			source: this.hostElement
		});
		if (this.trackSmall !== void 0) statusNote({
			tag: "deprecated",
	...
	getRangeAriaValueText() {
		return this.innerAriaValueText.replace(/\$from/g, `${this.valueFrom}`).replace(/\$to/g, `${this.valueTo}`);
	}
	render() {
		const helperTextId = `helper-message-${this.internalId}`;
		const ariaDescribedByAttr = { "aria-describedBy": helperTextId };
		return h(Host, null, this.styles && h("style", null, this.styles), h("div", { part: classnames("base", this.disabled && "disabled") }, h("div", { part: "label-wrapper" }, !!this.label && h("label", {
			part: "label",
			id: `${this.sliderId}-label`,
			htmlFor: this.sliderId
		}, this.label), this.showValue && h("div", { part: "value-text" }, this.getTextValue())), h("div", { part: "track-wrapper" }, h("div", {
			part: "track",
			ref: (el) => this.sliderTrack = el
		}, h("div", {
			part: "bar",
			style: {
		...
		}), this.showStepMarks && h("div", { part: "step-marks" }, this.getNumberOfSteps().map(() => h("span", { part: "step-mark" }))), h("div", { part: "inner-track" }, this.range ? h(Fragment, null, h("div", {
		...
		}, this.helperText ? ariaDescribedByAttr : {}, { onKeyDown: this.onKeyDown }))), h("div", {
			...
			"aria-orientation": "horizontal",
			"aria-disabled": this.disabled
		}, this.helperText ? ariaDescribedByAttr : {}, { onKeyDown: this.onKeyDown })))))), h("input", {
			type: "hidden",
...
//# sourceMappingURL=scale-slider.entry-B9DjCqiZ.js.map