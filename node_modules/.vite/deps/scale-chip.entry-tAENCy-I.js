...
//#region node_modules/@telekom/scale-components/dist/esm/scale-chip.entry.js
...
var Chip = class {
	constructor(hostRef) {
		...
		this.handleKeyDown = (event) => {
			if (event.code === "Space") this.handleChange(event);
		};
		this.handleChange = (event) => {
			event.preventDefault();
			event.stopPropagation();
			if (this.disabled || this.type === "dynamic") return;
	...
	componentDidRender() {
		...
		if (iconSlot !== null) {
			if (iconSlot.children[0].getAttribute("size") === String(defaultIconSize)) iconSlot.children[0].setAttribute("size", String(16));
			if (this.selected) iconSlot.children[0].setAttribute("selected", String(true));
			else iconSlot.children[0].setAttribute("selected", String(false));
		}
	}
	disconnectedCallback() {}
	getIcon() {
		if (this.type === "dynamic" && this.selected) return h("button", {
		...
		else if (this.type === "persistent" && this.selected) return h("scale-icon-action-checkmark", {
			"accessibility-title": "success",
			size: 16,
			selected: true
		});
		else if (this.type === "persistent") return h("scale-icon-action-checkmark", {
			"accessibility-title": "success",
			size: 16
		});
	}
	render() {
		return h(Host, null, this.styles && h("style", null, this.styles), h("span", {
		...
		}, h("slot", { name: "chip-icon" }), h("span", { class: "chip-label" }, h("slot", null)), this.selected ? this.getIcon() : null));
	}
	getBasePartMap() {
...
Chip.style = chipCss;
//#endregion
export { Chip as scale_chip };

//# sourceMappingURL=scale-chip.entry-tAENCy-I.js.map