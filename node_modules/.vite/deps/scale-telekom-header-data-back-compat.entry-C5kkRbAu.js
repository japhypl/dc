import { l as registerInstance, s as h } from "./index-6d95a4bc-DlO5twzZ.js";
...
//#region node_modules/@telekom/scale-components/dist/esm/scale-telekom-header-data-back-compat.entry.js
...
var TelekomHeaderDataBackCompat = class {
	...
	render() {
		...
		return h("scale-telekom-header", {
		...
		}, !readData(this.sectorNavigation) ? null : h("scale-telekom-nav-list", {
			slot: "meta-nav-external",
			variant: "meta-nav-external",
			alignment: "left"
		}, readData(this.sectorNavigation).map((item) => {
			return h("scale-telekom-nav-item", null, h("a", {
		...
		})), !readData(this.addonNavigation) ? null : h("scale-telekom-nav-list", {
			slot: "meta-nav",
			variant: "meta-nav",
			alignment: "right"
		...
		})), !readData(this.mainNavigation) ? null : h("scale-telekom-nav-list", {
			variant: "main-nav",
			slot: "main-nav"
			...
			return h("scale-telekom-nav-item", { active: isActive(item.id) }, h("a", {
				href: item.href || "javascript:void(0);",
				id: item.id,
				target: item.target || "_self",
				onClick: (event) => {
				...
				return h("scale-telekom-mega-menu-column", null, h("a", {
					href: child.href || "javascript:void(0);",
					target: child.target || "_self",
					onClick: (event) => {
						if (typeof child.onClick === "function") child.onClick(event);
					},
					slot: "heading"
				}, child.name), !child.children ? null : h("ul", null, child.children.map((grandChild) => {
					return h("li", null, h("a", {
						href: grandChild.href || "javascript:void(0);",
						target: grandChild.target || "_self",
						onClick: (event) => {
							if (typeof grandChild.onClick === "function") grandChild.onClick(event);
						}
					}, grandChild.name));
				})));
			}))));
		})), !readData(this.iconNavigation) && !readData(this.userNavigation) ? null : h("scale-telekom-nav-list", {
			variant: "functions",
			slot: "functions",
			alignment: "right"
		}, readData(this.userNavigation).length > 0 && h("scale-telekom-nav-item", { class: "user-menu-desktop" }, h("a", {
		...
		}, h("scale-menu-flyout", { direction: "bottom-left" }, badge ? h("scale-badge", {
			count: badgeLabel,
			label: shortName,
			"label-visually-hidden": true
		}, h("scale-icon-user-file-user", null, " ")) : h("scale-icon-user-file-user", null, " "), h("scale-menu-flyout-list", null, h("app-navigation-user-menu", {
			hide: () => {
				this.userMenuDesktopTrigger.click();
		...
		})))), readData(this.userNavigation).length > 0 && h("scale-telekom-nav-item", { class: "user-menu-mobile" }, h("button", { ref: (el) => {
			this.userMenuMobileTrigger = el;
		} }, h("scale-badge", {
			count: badgeLabel,
			label: shortName,
			"label-visually-hidden": true
		}, h("scale-icon-user-file-user", null, " "))), h("scale-telekom-nav-flyout", { variant: "mobile" }, h("scale-telekom-mobile-flyout-canvas", null, h("app-navigation-user-menu", {
			...
			navigation: readData(this.userNavigation)
		})))), (readData(this.iconNavigation) || []).filter(({ id }) => id !== "menu").map((item) => {
			return h("scale-telekom-nav-item", null, h("a", {
				href: item.href || "javascript:void(0);",
				target: item.target || "_self",
				id: item.id,
		...
		}), !readData(this.mainNavigation) && !readData(this.sectorNavigation) && !readData(this.addonNavigation) ? null : h("scale-telekom-nav-item", { "hide-on-desktop": true }, h("button", null, h("scale-badge", null, h("scale-icon-action-menu", null))), h("scale-telekom-nav-flyout", { variant: "mobile" }, h("scale-telekom-mobile-flyout-canvas", {
			"app-name": this.appName,
			"app-name-link": this.appNameLink || "javascript:void(0);",
			"app-name-click": (event) => {
				if (typeof this.appNameClick === "function") this.appNameClick(event);
			}
			...
			return h("scale-telekom-mobile-menu-item", {
				...
				}, child.name), !child.children ? null : child.children.map((grandChild) => h("scale-telekom-mobile-menu-item", {
					slot: "children",
...
//#endregion
export { TelekomHeaderDataBackCompat as scale_telekom_header_data_back_compat };

//# sourceMappingURL=scale-telekom-header-data-back-compat.entry-C5kkRbAu.js.map