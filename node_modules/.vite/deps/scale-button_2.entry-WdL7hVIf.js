...
import { i as hasShadowDom, s as isScaleIcon } from "./utils-c4af5b47-5j7RUTeV.js";
//#region node_modules/@telekom/scale-components/dist/esm/scale-button_2.entry.js
...
var Button = class {
		...
		this.handleClick = (ev) => {
			if (hasShadowDom(this.hostElement)) {
				...
				if (parentForm) {
					...
					if (this.type) fakeButton.type = this.type;
					if (this.value) fakeButton.value = this.value;
					if (this.name) fakeButton.name = this.name;
	...
	*/
	appendEnterKeySubmitFallback() {
		if (hasShadowDom(this.hostElement)) {
			...
			if (parentForm.querySelector("input[type=\"submit\"]") != null) return;
	...
	cleanUpEnterKeySubmitFallback() {
		if (this.fallbackSubmitInputElement != null) try {
	...
	*/
	setIconPositionProp() {
		...
		if (!this.iconOnly && lastNode && isScaleIcon(lastNode)) this.iconPosition = "after";
	}
	/**
	* Set any children icon's size according the button size.
	*/
	setChildrenIconSize() {
		if (this.size != null && buttonIconSizeMap[this.size] != null) Array.from(this.hostElement.childNodes).filter(isScaleIcon).forEach((icon) => {
			if (icon.size === DEFAULT_ICON_SIZE) icon.size = buttonIconSizeMap[this.size];
		});
	}
	render() {
...
var Icon = class {
	...
	render() {
		...
		return h(Host, null, h("svg", Object.assign({
			...
			viewBox: "0 0 24 24",
			role: "img"
		}, ariaHidden, focusable), this.accessibilityTitle && h("title", null, this.accessibilityTitle), this.path ? h("path", Object.assign({ d: this.path }, pathAttributes, { part: "path" })) : h("use", Object.assign({ xlinkHref: `#icon-${this.name}` }, pathAttributes))));
	}
	getCssClassMap() {
...
//# sourceMappingURL=scale-button_2.entry-WdL7hVIf.js.map