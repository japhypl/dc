...
var TelekomProfileMenu = class {
	constructor(hostRef) {
		registerInstance(this, hostRef);
		this.menuOpen = false;
	}
	onKeydown(event) {
		if (this.menuOpen && "Escape" === event.key) this.userMenuDesktopTrigger.click();
	}
	openMenu(event) {
		if (event.target.id === "user-menu-desktop") this.menuOpen = true;
	}
	closeMenu(event) {
		if (event.target.id === "user-menu-desktop") this.menuOpen = false;
	}
	printSignInMenu() {
		return h("div", { class: "profile-menu-login" }, h("strong", null, this.serviceName), h("p", null, this.serviceDescription), h("scale-button", { href: this.loginUrl || LOGIN_DEFAULT }, this.loginLabel), h("div", { class: "footer" }, h("p", null, h("scale-link", {
			"omit-underline": "true",
			href: this.loginHelpUrl || LOGIN_HELP_DEFAULT
		}, this.loginHelpLabel)), h("div", { id: "signUp" }, h("p", null, this.registerHeadline), h("p", null, h("scale-link", {
			"omit-underline": "true",
			href: this.registerUrl || REGISTER_DEFAULT
		}, this.registerLabel)))));
	}
	printProfileTrigger() {
		if (!this.loggedIn) return h("scale-icon-user-file-user", { selected: this.menuOpen });
		return h("scale-badge", { "no-dot": "true" }, h("scale-icon-user-file-user", { selected: this.menuOpen }), h("div", {
			slot: "dot",
			class: "mydot"
	...
	buildUserNavigation() {
		...
		if (userInfo) userInfo.type = "userInfo";
		...
		let menu = [];
		if (userInfo) menu = menu.concat(userInfo);
		if (userInfo && !this.serviceLinksEmpty()) menu = menu.concat(divider);
	...
	serviceLinksEmpty() {
		return this.hideLoginSettings && (!this.serviceLinks || this.serviceLinks.length < 1);
	}
	buildDesktopMenuStyles() {
		...
		if (this.serviceLinksEmpty()) style += "scale-button { margin-top: 32px !important; }";
		return style;
	}
	...
	printLabel() {
		if (!this.accessibilityLabel) return h("span", { class: "flyout-label" }, this.label);
		return h("div", { class: "flyout-label" }, h("span", { "aria-hidden": "true" }, this.label), h("span", { class: "visually-hidden" }, this.accessibilityLabel));
	}
	render() {
		return h(Host, null, h("scale-telekom-nav-item", { class: "user-menu-desktop" }, h("scale-menu-flyout", {
			direction: "bottom-left",
			"onScale-open": (event) => this.openMenu(event),
			"onScale-close": (event) => this.closeMenu(event),
		...
		}, this.printProfileTrigger(), this.printLabel()), h("scale-menu-flyout-list", {
			id: "user-menu-desktop",
			preventFlipVertical: true,
			role: "none"
		}, this.loggedIn && [h("app-navigation-user-menu", {
			hide: () => {
				this.userMenuDesktopTrigger.click();
			},
			navigation: this.buildUserNavigation(),
			styles: this.buildDesktopMenuStyles()
		})], !this.loggedIn && [h("app-navigation-user-menu", { navigation: [] }, this.printSignInMenu())]), h("div", {
			slot: "trigger",
			class: "user-menu-trigger",
			ref: (el) => this.userMenuDesktopTrigger = el
		}))), h("scale-telekom-nav-item", { class: "user-menu-mobile" }, h("button", null, this.printProfileTrigger(), this.printLabel()), h("scale-telekom-nav-flyout", { variant: "mobile" }, h("scale-telekom-mobile-flyout-canvas", {
...
//#endregion
export { TelekomProfileMenu as scale_telekom_profile_menu };

//# sourceMappingURL=scale-telekom-profile-menu.entry-C9Y1tuUf.js.map